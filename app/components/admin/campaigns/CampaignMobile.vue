    ...
    <div class="md:hidden space-y-3">
        <div v-for="campaign in campaigns" :key="campaign.id" class="bg-card rounded-lg border p-4 space-y-3">
            ...
            <div class="pt-2 border-t flex gap-2">
                <Button variant="outline" size="sm" class="flex-1" @click="$emit('edit', campaign)">
                    <Pencil class="h-3 w-3 mr-1" />
                    编辑
                </Button>
                <Button variant="outline" size="sm" @click="$emit('toggle-status', campaign)">
                    <component :is="campaign.status === 1 ? Pause : Play" class="h-3 w-3" />
                </Button>
                <Button variant="outline" size="sm" @click="$emit('delete', campaign)">
