...
<template>
    <Transition name="view-fade" mode="out-in">
        ...
        <div v-if="viewMode === 'grid'" key="grid" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            <component :is="isFreeform(d) ? 'div' : 'NuxtLink'" v-for="d in items" :key="d.id"
                :to="isFreeform(d) ? undefined : `/dashboard/document/drafts/${d.id}`"
                :role="isFreeform(d) ? 'button' : undefined"
                :tabindex="isFreeform(d) ? 0 : undefined"
                ...
                @click="isFreeform(d) && openPreview(d)"
                @keydown.enter="isFreeform(d) && openPreview(d)">
                <div
                    ...
                    <component :is="isFreeform(d) ? FileTextIcon : FileEditIcon" class="size-6" />
                </div>
        ...
        <div v-else key="list" class="space-y-1">
            <component :is="isFreeform(d) ? 'div' : 'NuxtLink'" v-for="d in items" :key="d.id"
                ...
                <div
                    ...
                    <component :is="isFreeform(d) ? FileTextIcon : FileEditIcon"
