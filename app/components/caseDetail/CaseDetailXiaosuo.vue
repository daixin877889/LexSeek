...
<template>
  ...
  <template v-if="!isMobile">
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
      ...
      <div v-if="isOpen && isFullscreen" class="fixed md:absolute inset-0 z-50 bg-background flex flex-col">
            ...
            <Popover v-model:open="sessionListOpen">
              <PopoverTrigger as-child>
                <button class="flex items-center gap-1 text-sm font-medium hover:text-primary transition-colors">
                  小索 - {{ currentSessionTitle }}
                  <ChevronDownIcon class="size-3.5" />
                </button>
              </PopoverTrigger>
              <PopoverContent class="w-64 p-0" align="start">
                <div class="max-h-60 overflow-y-auto">
                  <div v-for="s in xiaosuoChat.sessions.value" :key="s.sessionId"
                    ...
                    <button class="shrink-0 ml-1 p-1 rounded hover:bg-destructive/10 hover:text-destructive"
        ...
        <div class="flex-1 overflow-hidden">
          <AiChat :messages="chatMessages" :loading="chatLoading" panel-mode="left" :show-header="false"
            v-model:thinking="thinking" :enable-file-upload="false" prompt-placeholder="问我任何关于案件的问题..."
            @submit="handleSubmit" @stop="xiaosuoChat.stopGeneration()" />
        </div>
      </div>
    </Transition>
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0"
      ...
      <div v-if="isOpen && !isFullscreen"
        ...
        :class="{ 'select-none': isInteracting }" :style="containerStyle" @pointermove="onEdgeDetect"
        @pointerdown="onResizeStart">
        <!-- 小窗标题栏（可拖拽） -->
        <div class="shrink-0 h-10 flex items-center justify-between px-3 border-b bg-muted/30
                    cursor-grab active:cursor-grabbing" @pointerdown="onDragStart">
          <div class="flex items-center gap-2">
              ...
              <PopoverContent class="w-56 p-0" align="start">
                <div class="max-h-48 overflow-y-auto">
                  <div v-for="s in xiaosuoChat.sessions.value" :key="s.sessionId"
                    ...
                    <span class="truncate flex-1">{{ s.title }}</span>
                    <span class="shrink-0 text-xs text-muted-foreground mx-1">{{ dayjs(s.updatedAt).fromNow() }}</span>
                    <button class="shrink-0 ml-1 p-0.5 rounded hover:bg-destructive/10 hover:text-destructive"
                      @click.stop="handleDeleteSession(s.sessionId)">
    ...
    <div class="absolute bottom-4 right-4 z-40">
      <img v-show="!isFullscreen" :src="xiaosuoIcon"
        class="size-12 cursor-pointer hover:scale-110 transition-transform drop-shadow-lg" alt="小索"
        @click="isOpen = !isOpen" />
    </div>
  </template>
  ...
  <template v-else>
    <Sheet v-model:open="isOpen">
      <SheetContent side="bottom" class="h-[90vh] flex flex-col p-0">
                ...
                <PopoverContent class="w-64 p-0" align="start">
                  <div class="max-h-60 overflow-y-auto">
                    <div v-for="s in xiaosuoChat.sessions.value" :key="s.sessionId"
                      ...
                      @click="handleSwitchSession(s.sessionId)">
                      <span class="truncate flex-1">{{ s.title }}</span>
                      <span class="shrink-0 text-xs text-muted-foreground mx-1">{{ dayjs(s.updatedAt).fromNow()
                      }}</span>
                      <button class="shrink-0 ml-1 p-1 rounded hover:bg-destructive/10 hover:text-destructive"
                        @click.stop="handleDeleteSession(s.sessionId)">
                        <Trash2Icon class="size-3" />
        ...
        <div class="flex-1 overflow-hidden">
          <AiChat :messages="chatMessages" :loading="chatLoading" panel-mode="left" :show-header="false"
            :show-thinking-toggle="false" :enable-file-upload="false" prompt-placeholder="问我任何关于案件的问题..."
            @submit="handleSubmit" @stop="xiaosuoChat.stopGeneration()" />
        </div>
      </SheetContent>
    </Sheet>
  ...
  <Dialog :open="!!interruptData" @update:open="() => { }">
    <DialogContent class="sm:max-w-md">
      <InitAnalysisInsufficientPointsCard v-if="interruptData" :is-member="interruptData.data?.isMember ?? false"
        :available-points="interruptData.data?.availablePoints" :required-points="interruptData.data?.requiredPoints"
        :reason="interruptData.data?.reason" @resume="resumeWorkflow" />
    </DialogContent>
  </Dialog>
</template>
