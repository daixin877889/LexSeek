<template>
    ...
    <div class="hidden md:grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        <div v-if="list.length === 0" class="col-span-full py-12 text-center text-muted-foreground bg-card border border-dashed rounded-xl">
            没有找到匹配的案件记录
        </div>
        ...
        <div v-for="item in list" :key="item.id" 
            ...
            <div class="p-5 flex-1 space-y-4">
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="flex flex-col items-end gap-1.5 shrink-0">
                        <UiBadge :class="getStatusBadgeClass(item.status)" variant="outline" class="rounded-md border-transparent px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                            {{ getStatusText(item.status) }}
                        </UiBadge>
                        <UiBadge v-if="item.isDemo" variant="secondary" class="rounded-md bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 text-[9px] font-bold uppercase px-1.5 py-0">
                            演示
                        </UiBadge>
                    </div>
                </div>
            ...
            <div class="px-5 py-3 bg-muted/30 border-t border-border/50 flex items-center justify-between mt-auto">
                ...
                <NuxtLink :to="`/case/analysis/${item.id}`">
                    <Button variant="link" size="sm" class="h-8 p-0 text-muted-foreground hover:text-primary font-bold hover:no-underline transition-colors flex items-center gap-1 group/btn">
