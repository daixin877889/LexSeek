    ...
    <div class="max-w-[1280px] mx-auto px-4 py-3.5 flex items-center justify-between">
      ...
      <div class="flex items-center gap-9">
        <NuxtLink to="/" class="flex items-center gap-2.5">
          ...
          <span translate="no" class="text-[19px] font-bold whitespace-nowrap">
            LexSeek<span class="text-muted-foreground font-normal mx-1.5">｜</span>法索 AI
          </span>
        </NuxtLink>
        <nav class="hidden md:block">
          <ul class="flex items-center gap-1.5">
            <li v-for="link in NAV_LINKS" :key="link.to">
              <NuxtLink
                :to="link.to"
                class="block px-3.5 py-2 rounded-md text-sm transition-colors"
      ...
      <div class="flex items-center gap-2">
        ...
        <div v-if="!authStore.isAuthenticated" class="hidden md:flex items-center gap-1">
          <NuxtLink
            :to="loginLink"
            class="px-3.5 py-2 text-sm font-medium text-foreground hover:text-primary transition-colors"
          >登录</NuxtLink>
          <NuxtLink
            :to="loginLink"
          ...
          <SheetContent side="left" class="w-[280px] sm:max-w-[280px] p-0 gap-0">
            ...
            <nav class="flex-1 flex flex-col gap-1 p-3">
              <NuxtLink
                ...
                :key="link.to"
                :to="link.to"
                class="px-3.5 py-3 rounded-md text-[15px] transition-colors"
                :class="isActive(link.to)
                  ? 'text-primary font-semibold bg-primary/10'
                  : 'text-foreground font-medium hover:bg-primary/5'"
