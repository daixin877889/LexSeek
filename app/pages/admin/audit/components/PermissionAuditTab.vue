<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row gap-4">
      ...
      <Button variant="ghost" :class="adminBrandFocusClass" @click="resetFilters">
        ...
        <Table>
          <TableHeader>
            <TableRow class="bg-muted/50 hover:bg-muted/50">
              <TableHead class="w-40 px-4 py-3">操作类型</TableHead>
              <TableHead class="w-28 px-4 py-3">目标类型</TableHead>
              <TableHead class="w-20 px-4 py-3">目标ID</TableHead>
              <TableHead class="px-4 py-3">操作人</TableHead>
              <TableHead class="px-4 py-3">备注</TableHead>
              <TableHead class="w-44 px-4 py-3">操作时间</TableHead>
              <TableHead class="w-20 px-4 py-3 text-center">详情</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            <TableRow v-for="log in logs" :key="log.id" class="hover:bg-muted/30">
              <TableCell class="px-4 py-3">
                <Badge variant="outline" :class="getActionBadgeClass(log.action)">{{ getActionLabel(log.action) }}</Badge>
              </TableCell>
              <TableCell class="px-4 py-3">
                <Badge variant="outline" :class="adminBrandChipClass">{{ getTargetTypeLabel(log.targetType) }}</Badge>
              </TableCell>
              <TableCell class="px-4 py-3 text-sm">{{ log.targetId || '-' }}</TableCell>
              <TableCell class="px-4 py-3 text-sm">{{ log.operator?.name || log.operator?.phone || '-' }}</TableCell>
              <TableCell class="px-4 py-3 text-sm text-muted-foreground max-w-xs truncate">{{ log.remark || '-' }}</TableCell>
              <TableCell class="px-4 py-3 text-sm text-muted-foreground">{{ formatDate(log.createdAt) }}</TableCell>
              <TableCell class="px-4 py-3 text-center">
                <Button variant="ghost" size="icon" :class="['h-8 w-8', adminBrandFocusClass]" @click="showDetail(log)">
                  <Eye class="h-4 w-4" />
                </Button>
              </TableCell>
            </TableRow>
  ...
  <Dialog v-model:open="detailDialogOpen">
    <DialogContent class="theme-brand max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
      <DialogHeader class="shrink-0">
        <DialogTitle>审计日志详情</DialogTitle>
      </DialogHeader>
      <div v-if="selectedLog" class="min-h-0 flex-1 overflow-y-auto space-y-4 py-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <Label class="text-muted-foreground">操作类型</Label>
            <div class="mt-1">
              <Badge variant="outline" :class="getActionBadgeClass(selectedLog.action)">{{ getActionLabel(selectedLog.action) }}</Badge>
            </div>
          </div>
          <div>
            <Label class="text-muted-foreground">目标类型</Label>
            <div class="mt-1">
              <Badge variant="outline" :class="adminBrandChipClass">{{ getTargetTypeLabel(selectedLog.targetType) }}</Badge>
            </div>
          </div>
          <div>
...
const getActionBadgeClass = (action: string): string => {
  if (action.includes('delete')) return adminBrandErrorBadgeClass
  if (action.includes('create')) return adminBrandEnabledBadgeClass
  if (action.includes('batch')) return adminBrandWarningBadgeClass
  if (action.includes('update') || action.includes('assign')) return adminBrandActiveBadgeClass
