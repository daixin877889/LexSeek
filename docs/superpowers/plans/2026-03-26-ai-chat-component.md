...
同时迁移积分/会员购买相关组件：
- 将 `app/components/initAnalysis/InsufficientPointsCard.vue` 复制到 `app/components/ai/tools/InsufficientPointsCard.vue`
- 将 `app/components/case/interrupt/InsufficientPointsHandler.vue` 复制到 `app/components/ai/tools/InsufficientPointsHandler.vue`

这两个组件是积分不足中断时的购买界面（含会员套餐选择和积分商品购买），直接迁移到 `ai/tools/` 下。
