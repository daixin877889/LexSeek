...
在组件内定义可复用的标题栏模板（全屏和小窗共用 session 切换逻辑），使用 Popover 组件。在 `</script>` 后添加模板。完整模板较长，核心结构如下：
...
- [ ] **Step 3: 替换消息区域为 AiChat**

全屏、小窗、移动端三处的消息列表+输入框统一替换为：
...
注意：`xiaosuoChat.messages` 和 `xiaosuoChat.isLoading` 是 `ComputedRef`，在模板中需要 `.value` 展开（与 `AnalysisModuleChat.vue` 中 `chatInstance.messages.value` 的写法一致）。

- [ ] **Step 4: 添加中断处理 Dialog**

在模板最后（悬浮按钮之后）添加积分不足弹窗：
