...
> ```
>
> **响应映射**（必须保留 `moduleName`/`nodeId` 并新增 `title`/`createdAt`/`updatedAt`）：
> ```typescript
> const result = sessions.map(s => ({
>     sessionId: s.sessionId,
>     moduleName: (s.metadata as any)?.moduleName,
>     nodeId: (s.metadata as any)?.nodeId,
>     title: (s.metadata as any)?.title ?? (s.metadata as any)?.moduleName,
...
>
> 这个格式同时满足两个消费方：
> - `useChatSessionManager.fetchSessions()` 需要 `SessionItem`（sessionId/title/createdAt/updatedAt/hasActiveRun）
> - `useModuleChatManager.restoreActiveSessions()` 需要 `moduleName`（按模块分组）
...
export function useModuleChatManager(caseId: Ref<number>, options: ModuleChatManagerOptions = {}) {
    ...
    async function restoreActiveSessions() {
        // 不带 moduleName 过滤，获取所有模块的 session（按模块分组）
...
### Task 9.5: PR #2 Composable 单元测试

**Files:**
- Create: `tests/client/composables/useStreamChat.test.ts`
- Create: `tests/client/composables/useChatSessionManager.test.ts`

> Spec Section 7.1 要求 PR #2 包含 composable 单元测试。参考项目已有的 `tests/client/composables/` 下的测试模式。

- [ ] **Step 1: 编写 useStreamChat 单元测试**

测试场景：
- `interruptData`：mock useStream 返回含 `__interrupt__` 的 values getter，验证解包逻辑（数组→首项→.value）
- `interruptData`：**确认不使用 stream.interrupt**（只从 values.__interrupt__ 路径读取）
- `messages` computed：验证 `void stream.values` 触发 track 后 messages 正确返回
- `hasHistoryLoaded`：验证 watch(values) 首次非空时标记为 true
- `reconnect` / `loadHistory`：验证重置 hasHistoryLoaded + 调用 submit(undefined)

> Mock `@langchain/vue`：返回一个对象含 ES6 getter（模拟 useStreamCustom 行为）+ shallowRef（isLoading/error）。参考 `tests/client/composables/useCaseAnalysis.test.ts` 了解已有的 mock 模式。

- [ ] **Step 2: 编写 useChatSessionManager 单元测试**

测试场景：
- `init`：幂等保护（调用两次只执行一次 fetchSessions）
- `switchSession`：竞态防护（快速连续调用两次，第一次的 scope 被清理）
- `stopGeneration`：双重取消（stream.stop + stopActiveRun）
- `createSession`：列表头部追加 + 自动 switchSession
- `deleteSession`：从列表移除 + 当前 session 被删时自动切换
- `renameSession`：调用 API + 本地列表乐观更新

> Mock `useApiFetch` 和 `useCaseChat`（Nuxt 自动导入的全局变量）。

- [ ] **Step 3: 运行测试**

Run: `npx vitest run tests/client/composables/useStreamChat.test.ts tests/client/composables/useChatSessionManager.test.ts --reporter=verbose`
Expected: PASS

- [ ] **Step 4: 提交**

```bash
git add tests/client/composables/useStreamChat.test.ts \
    tests/client/composables/useChatSessionManager.test.ts
git commit -m "test(composable): useStreamChat + useChatSessionManager 单元测试"
```
...
**Files:**
- Create: `tests/client/composables/useInitAnalysis.comparison.test.ts`
...
Run: `npx vitest run tests/client/composables/useInitAnalysis.comparison.test.ts --reporter=verbose`
Expected: PASS（基线通过）

- [ ] **Step 3: 提交**

```bash
git add tests/client/composables/useInitAnalysis.comparison.test.ts
...
> 5. **属性**（`stream.values`/`stream.messages`/`stream.isLoading`）改为 `stream.xxx.value` 访问；**方法**（`stream.submit`/`stream.stop`/`stream.reconnect`/`stream.loadHistory`）**保持直接调用**（不加 `.value`）
...
> 3. **属性**（`stream.values`/`stream.messages`/`stream.isLoading`）改为 `.value` 访问；**方法**（`submit`/`stop`）保持直接调用（与 Task 14 相同）
...
```
PR #1 (Backend DAO):
  Task 1 → Task 2 → Task 3 → Task 4 → Task 4.5
                                         │
PR #2 (Frontend Composable):             │
  Task 5 → Task 6 → Task 7 → Task 8 → Task 9 → Task 9.5
                                                    │
                         ┌──────────────────────────┴──────────────┐
