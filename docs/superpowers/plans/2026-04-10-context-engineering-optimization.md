...
> - 实施时需先确认 `caseTypeId` 字段是否存在，可能是 `caseType` 而非 `caseTypeId`
...
在 `stripSystemMessages` 函数中（第 536 行）添加 `updates` 事件分支，复用 `isInternalMessage`：

```typescript
function stripSystemMessages(event: string, data: unknown): unknown | null {
    if (!data || typeof data !== 'object') return data

    // values 和 updates 事件：过滤 data.messages 数组
