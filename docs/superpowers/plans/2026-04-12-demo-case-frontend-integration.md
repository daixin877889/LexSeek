...
在 `shared/types/case.ts` 末尾或合适位置添加：
...
export interface DemoCasePrepareResponse {
  content: string | null
  files: import('~/store/file').OssFileItem[]
}
```
...
git commit -m "feat(types): 新增示范案例列表与 prepare 响应类型"
```
...
/** 将 Prisma ossFile 行转成前端用的 OssFileItem */
function toOssFileItem(file: {
    id: number
    fileName: string
    fileSize: any
    fileType: string
    ...
    return {
        ...
        source: (file.source ?? FileSource.CASE_ANALYSIS) as string,
        sourceName: '示范案例',
        status: file.status,
        statusName: '正常',
            ...
            for (const material of materials) {
                ...
                if (existing) {
                    if (existing.deletedAt !== null) {
                        ...
                        result.push(toOssFileItem(revived))
                    } else {
...
在 `index.post.ts` 和 `[id].put.ts` 中将 `materials` 字段替换为：
...
- `getDefaultForm` 返回包含 `content: ''` 和 `materials: []`
- `openEdit` 时从 `item.content` 和 `item.materials` 填充
...
在 `useCaseCreation` 函数返回对象前新增：

```ts
const demoCases = ref<import('#shared/types/case').DemoCaseListItem[]>([])
const demoCasesLoading = ref(true)
const preparingDemoCaseId = ref<number | null>(null)
const showReplaceConfirm = ref(false)
const pendingExample = ref<import('#shared/types/case').DemoCaseListItem | null>(null)

// promptInputRef 由调用方传入（暴露 setText / addFiles / reset / hasContent）
const promptInputRef = ref<{
  setText: (v: string) => void
  addFiles: (files: import('~/store/file').OssFileItem[]) => void
  reset: () => void
  hasContent: () => boolean
} | null>(null)
...
async function loadDemoCases() {
  ...
  try {
    const data = await useApiFetch<{ items: import('#shared/types/case').DemoCaseListItem[] }>(
      '/api/v1/demo-cases',
    )
...
async function fetchAndFillDemoCase(example: import('#shared/types/case').DemoCaseListItem) {
  const data = await useApiFetch<import('#shared/types/case').DemoCasePrepareResponse>(
...
async function applyDemoCase(example: import('#shared/types/case').DemoCaseListItem) {
...
async function handleExampleSelect(example: import('#shared/types/case').DemoCaseListItem) {
...
- [ ] **Step 1: 更新 script setup 的 composable 解构**
...
const {
  ...
  promptInputRef: composablePromptInputRef,
  loadDemoCases, handleExampleSelect, confirmReplaceExample,
} = useCaseCreation()
```

- [ ] **Step 2: 把本地 `promptInputRef` 绑定到 composable 的 ref**

```ts
const promptInputRef = ref()
watchEffect(() => {
  composablePromptInputRef.value = promptInputRef.value
})
```
...
Run: `grep -rn "DemoCaseList" app/ server/ --include="*.vue" --include="*.ts" | grep -v "app/components/case/DemoCaseList.vue"`
Expected: 无输出（或仅 DemoCaseList.vue 自身）
