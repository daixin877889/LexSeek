...
async function buildDocx(text: string): Promise<Buffer> {
    // 最小 .docx 构造：用 mammoth 不支持写，改用 docx 库或 pre-made fixture
    // 这里用 fixture：放 3-5 个小 .docx 到 tests/fixtures/document-templates/
    throw new Error('TODO: 用 fixture 文件实现')
}
...
注：fixture .docx 文件需在本步附带生成。**Step 1.5: 先生成 fixture**：写 5 个最小 .docx 到 `tests/fixtures/document-templates/`（可用 `docx` 库脚本生成；保持 ≤ 1KB）
...
**Files:**
- Create: `prisma/seeds/document-templates/*.docx`（至少 5 个：litigation / arbitration / general 三类）
- Modify: `prisma/seed.ts` 追加 `seedDocumentTemplates`
- Modify: `prisma/seeds/seedData.sql` 同步（样本模板的 OSS 上传需 seed 运行时处理，SQL 里只补节点）

- [ ] **Step 1: 准备 5 个样本模板（产品已定 O1 = 起诉状/答辩状/委托代理合同/律师函/民事协议）**
...
```typescript
async function seedDocumentTemplates(prisma: PrismaClient): Promise<void> {
    ...
    for (const file of files) {
        ...
        const existing = await prisma.documentTemplates.findFirst({
        ...
        const ossFileId = await uploadBufferToOSS(buffer, `seed-templates/${file}`)

        await prisma.documentTemplates.create({
            data: {
                name,
                category: inferCategoryFromFilename(name), // 从文件名前缀推断
                scope: 'global',
                userId: null,
                ossFileId,
...
注：`inferCategoryFromFilename` 和 `uploadBufferToOSS` 需小工具；可简化为映射表 + 复用现有 OSS helper（从 `server/lib/oss/` 或 `server/services/storage/`）。
...
export async function createDocumentTemplateService(params: {
    ...
    // 3. 配额校验（scope=user 且 transaction 内串行）
    if (!params.isAdmin) {
        return await prisma.$transaction(async (tx) => {
            ...
            const ossFileId = await uploadBufferToOSS(params.file, `user-templates/${params.userId}/${params.fileName}`)
            const template = await tx.documentTemplates.create({ data: { /* ... */ scope: 'user', userId: params.userId } })
            return { templateId: template.id }
        })
    }

    // 4. admin 上传：scope=global
    const ossFileId = await uploadBufferToOSS(params.file, `global-templates/${params.fileName}`)
    const template = await prisma.documentTemplates.create({ data: { /* ... */ scope: 'global', userId: null } })
    return { templateId: template.id }
}
```
...
// 3. --dry-run 跳过写库
// 4. 否则 uploadBufferToOSS + documentTemplates.create(scope='global')
// 5. name 已存在则跳过（幂等）
```
