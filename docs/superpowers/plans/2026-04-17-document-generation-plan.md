...
- `DocumentDraftPanel.vue` — 主容器，支持 `:case-id` prop；**内联** runStatus 展示（旋转 + "AI 正在生成"文案），不单独拆组件
...
先写 4 个最小 .docx 到 `tests/fixtures/document-templates/`（可用 `docx` 库脚本生成；保持 ≤ 1KB）：
- `english.docx`：只含 `{{plaintiff_name}}` / `{{loan_amount}}` 等英文占位符
- `chinese.docx`：只含 `{{原告}}` / `{{借款金额}}` 等中文占位符
- `mixed.docx`：混合中英 + 重复占位符
- `empty.docx`：无占位符的普通文档（API 契约测试：上传空模板应 400）
...
> 注：`long-context` 用例删除（原测占位符所在段落超 200 字）——`slice(0, 200)` 的截断由 TypeScript 原生行为保证，单测意义有限。如上线后发现 context 截断导致 AI 推断错位，再加。
...
**用户视角**：律师暂无感知（纯铺垫里程碑）。内部产品可通过 Prisma Studio 看到 `document_templates` 里已预置 5 份样本模板。
...
- 用户上传成功（扫描占位符成功）
- 扫描无占位符 → 抛错
- 配额边界：串行上传 20 个成功 / 第 21 个被拒（**不单测并发；并发场景由 M7 E2E + Prisma `$transaction` 天然保证**）
...
**用户视角**：管理员可以登 admin 后台管理全局模板库；普通律师可以在"文书生成 > 私人模板"页上传自己常用的模板（上限 20 个）。但文书生成主功能还没通（等 M4）。
...
**用户视角**：律师仍然看不到任何文书生成 UI（等 M4），但**后端闭环已经可用**——产品可以通过 curl 命令演示："AI 已能按模板填占位符"，消除后续 M4 的可用性风险。
...
### Task 4.3: 5 个文书组件（DocumentRunStatus 合并入主容器）

**Files:**
- Create: `app/components/assistant/document/DocumentDraftPanel.vue`（内联 runStatus 展示）
...
- [ ] **Step 3: DocumentFieldForm**

按 §8.3 4 类控件推断：DatePicker / Number / Textarea (>50 chars) / Input
每个字段显示 `suggestions` tooltip（若存在）
值改变 → debounce 500ms → `onFieldChange`

- [ ] **Step 4: DocumentPreview（M4 占位版）**

显示 "导出预览 .docx" 按钮 → 调 `onExport` → 下载

- [ ] **Step 5: DocumentDraftPanel（主容器 + 内联 runStatus）**

接受 `:case-id` prop；组合上述 4 个子组件；通过 `useDocumentDraft(draftId)` 拿数据；**直接在 template 里内联 runStatus 展示**（无需独立组件）：

```vue
<template>
  <div>
    <DocumentTemplatePicker v-model:template-id="templateId" />
    <DocumentSourceInput :case-id="caseId" @submit="handleStart" />

    <!-- 内联 runStatus：只一行逻辑，不单独拆组件 -->
    <div v-if="runStatus === 'filling'" class="flex items-center gap-2 py-2 text-muted-foreground">
      <Loader2Icon class="size-4 animate-spin" />
      <span>AI 正在生成文书...</span>
    </div>

    <DocumentFieldForm v-if="runStatus === 'ready' || runStatus === 'exported'" v-model:values="draft.values" :template="template" :suggestions="draft.metadata?.suggestions" @change="onFieldChange" />
    <DocumentPreview v-if="draft && template" :template-buffer="templateBuffer" :values="draft.values" @export="onExport" />
  </div>
</template>
```
...
git commit -m "feat(assistant): 新增文书生成前端 5 组件 + composable（runStatus 内联入 DraftPanel）"
```
...
- Modify: `prisma/seed.ts` 或 `scripts/seed-admin-routers.ts`（菜单路由 path 迁移）

- [ ] **Step 1: 新建页面（必须含 `definePageMeta`）**

spec §10.1 约定路径 `/dashboard/assistant/document`。完整 `definePageMeta`（项目 ui.md 规范强制）：
...
- [ ] **Step 2: 菜单路由迁移（routers 表）**

检查菜单注册位置：

```bash
# 查 routers 表里的现有"文书生成"菜单
psql $DATABASE_URL -c "SELECT id, name, title, path FROM routers WHERE name LIKE '%document%' OR path LIKE '%document%';"
```

若现有菜单 path = `/dashboard/document`（原 WIP），需要 **UPDATE** 为 `/dashboard/assistant/document`。修改方式二选一：
- **方案 A**（推荐，写 seed）：在 `prisma/seed.ts` 加 idempotent 的"菜单 path 纠正"函数，按 name='dashboard-assistant-document'（或对应 name）update path
- **方案 B**（手工迁移一次）：直接 SQL `UPDATE routers SET path = '/dashboard/assistant/document' WHERE path = '/dashboard/document'`，且生产部署前执行

然后删掉旧 `app/pages/dashboard/document.vue`。

- [ ] **Step 3: 浏览器手测**：登录 → 侧栏"文书生成"菜单 → 跳转 `/dashboard/assistant/document` → 选模板 → 粘贴材料 → 点开始生成 → AI 填完 → 改字段 → 导出

- [ ] **Step 4: 菜单路由检查清单**

- [ ] 侧栏显示"文书生成"菜单
- [ ] 点击菜单跳 `/dashboard/assistant/document`（不是旧的 `/dashboard/document`）
- [ ] 直接访问旧路径 `/dashboard/document` → 404（或重定向到新路径）

- [ ] **Step 5: Commit**

```bash
git commit -m "feat(assistant): 新增独立文书生成页 + 菜单路由迁移"
```
...
**用户视角**：🎯 **律师首次可用**——侧栏点"文书生成"→ 选模板 → 粘贴或上传材料 → 点"开始生成" → 等 AI 填完 → 手动调整字段 → 点"导出"下载 .docx。**这是交付给律师的核心价值点**。
...
选 3-5 个样本模板各跑一轮"选模板 → 输入 → 生成 → 改字段 → 看预览"。**主观观感判断**：若多个模板出现样式错位、占位符残留、段落错乱 → 触发降级（走 Step 4）；若观感正常即可。不做精确失败率测量（节省工时）。
...
- [ ] 3-5 样本模板主观观感正常，无样式错位
- [ ] 若触发降级，已按方案 A/B 实施

**用户视角**：律师编辑字段时右侧预览实时同步跟随，见所得即所得。填字段过程更流畅。
...
**用户视角**：律师管理案件时，无需离开案件详情页，直接在"文书"tab 里选模板 + 勾选已有的案件材料 → 生成文书 → 一键存回案件材料库。覆盖"案件内文书生成"场景。
...
**用户视角**：运营完成首批 100+ 生产模板批量导入后，律师在模板选择页能看到按 9 分类组织的完整模板库。产品可上线。
