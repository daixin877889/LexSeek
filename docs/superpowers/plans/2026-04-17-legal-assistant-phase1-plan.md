...
**Goal:** 上线"通用问答 · 对话"页面，支持无 case 会话的持久化对话、积分门控、会话列表管理；不包含合同审查/文书生成（第二、三期）。
...
- 用于支撑通用问答的无 case 会话（spec §4.2/§4.3）"
```
...
async function seedAssistantTokenRule(prisma: PrismaClient) {
  ...
  await prisma.pointConsumptionRules.upsert({
    ...
    update: {},  // 不覆盖运营已设置的单价
    create: reference
      ? { ...reference, id: undefined, itemKey: 'assistant_token', itemName: '通用问答 token 计费' }
      : {
          itemKey: 'assistant_token',
          itemName: '通用问答 token 计费',
...
export async function generateSessionTitleAsync(
  ...
  try {
    ...
    const config = await getValidNodeConfig('assistantMain', '通用问答主Agent')
...
|---|---|---|---|---|
| `/dashboard/assistant/chat` | 通用问答 · 对话 | MessageSquare | null（或"通用问答"父项下）| 10 |
| `/dashboard/assistant/contract` | 通用问答 · 合同审查 | FileSearch | 同上 | 20 |
| `/dashboard/assistant/document` | 通用问答 · 文书生成 | FileText | 同上 | 30 |
...
git commit -m "feat(rbac): 注册通用问答三级菜单路由
...
`document.vue` 同构（改标题 `通用问答 · 文书生成`、文案 `文书生成功能正在建设中 / 第三期`）。

- [ ] **Step 3: 浏览器验证**

启动 dev server：`bun dev`

从 Task 16 注册好的侧边栏菜单点击"通用问答 · 对话"进入：
- 左侧显示"暂无会话"
- 点「新对话」创建成功
- 右侧输入框可用
- 发消息 → 看到流式响应

再点"通用问答 · 合同审查"和"文书生成"：看到 WIP 占位。
