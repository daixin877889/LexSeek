...
- [ ] **Step 1: 写失败测试 — 覆盖两个独立 DAO 函数的关键行为**
...
describe('updateDraftTitleIfNotOverriddenDAO (AI 安全写)', () => {
    ...
    it('overridden=true 时原子 UPDATE 未命中，返回 null', async () => {
        const draft = await createDocumentDraftDAO({
            userId: 1, templateId: 1, sessionId: 'sid-title-3',
            status: 'ready', values: {}, sourceRef: null, metadata: null, caseId: null,
        })
...
Run: `npx vitest run tests/server/assistant/document/documentDraft.dao.test.ts -t updateDraftTitle`
Expected: FAIL — 函数未定义

- [ ] **Step 3: 实现两个独立 DAO 函数 + 让 `createDocumentDraftDAO` 支持 title/titleOverridden**
...
git commit -m "feat(document): documentDraft.dao 支持 title 字段 + 用户/AI 两条独立更新路径"
```
...
describe('updateDraftTitleService', () => {
    ...
    it('owner 可更新标题并置 titleOverridden=true', async () => {
        ...
        expect(mocksDao.updateDraftTitleDAO).toHaveBeenCalledWith(1, 'X')
    })
...
describe('applyAITitleIfAllowedService', () => {
    ...
    it('titleOverridden=false 时正常写入', async () => {
        mocksDao.updateDraftTitleIfNotOverriddenDAO.mockResolvedValue({ id: 1, title: 'AI', titleOverridden: false })
        const ok = await applyAITitleIfAllowedService(1, 'AI')
        expect(ok).toBe(true)
        expect(mocksDao.updateDraftTitleIfNotOverriddenDAO).toHaveBeenCalledWith(1, 'AI')
    })

    it('原子 update 未命中（用户已改）时返 false，不抛错', async () => {
        mocksDao.updateDraftTitleIfNotOverriddenDAO.mockResolvedValue(null)
        const ok = await applyAITitleIfAllowedService(1, 'AI')
    ...
    it('空字符串直接跳过', async () => {
        ...
        expect(ok).toBe(false)
        expect(mocksDao.updateDraftTitleIfNotOverriddenDAO).not.toHaveBeenCalled()
    })
})
```
...
```ts
import { updateDraftTitleDAO, updateDraftTitleIfNotOverriddenDAO } from './documentDraft.dao'
```
...
/**
 * 创建快照并尝试清理最旧快照，保持 draftId 下总数 ≤ SNAPSHOT_KEEP。
 * 清理失败仅日志 warn 不阻塞主流程 —— 故 insert 与 prune 顺序执行、不包事务。
 */
...
git commit -m "feat(document): 新增 PATCH /drafts/[id]/title 接口"
```
...
git commit -m "feat(document): 新增快照列表 + 恢复接口"
```
...
git commit -m "feat(document): 版本 list/create/rename/delete 接口"
```
...
git commit -m "feat(document): 版本 restore + export 接口"
```
...
describe('DocumentDraftTitleInput', () => {
    ...
    it('相同内容不 emit save（避免无意义 PATCH）', async () => {
        const w = mount(DocumentDraftTitleInput, { props: { title: 'same' } })
        await w.get('[data-testid="title-display"]').trigger('click')
        const input = w.get('input')
...
接受 `currentValues / snapshot` → 渲染字段对比表（字段名自合并两边 keys 推断）；emit `applyField(name)` 与 `applyAll`。
...
// 统一的"破坏性操作"二次确认弹窗（用 shadcn AlertDialog）
