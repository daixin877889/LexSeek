...
**Goal:** cases 表新增 5 个诉讼信息字段（法院/一二审案号/一二审法官）、状态扩到 6 档、前端表单平铺新字段 + 状态下拉、AI 抽取双路回填、ARCHIVED 状态只读保护、三个列表文件统一 `CaseStatusBadge` 组件消除重复。

**Architecture:** Prisma migration 加字段 + shared/types/case.ts 改 enum；`caseExtraction.service.ts` 的 Zod schema 扩展 5 字段；前端 `ManualForm.vue` 平铺分区；新建 `CaseStatusBadge.vue` 组件替代三个列表文件里的重复 `getStatusText/getStatusBadgeClass`；服务端 `updateCaseService` / 分析触发处加 `status !== ARCHIVED` 守卫。
...
### 新建
- `app/components/cases/CaseStatusBadge.vue` — 统一 6 档状态徽章
...
- `app/components/cases/CasesTable.vue` — 用 CaseStatusBadge 替换
- `app/components/cases/CasesGrid.vue` — 同上
- `app/components/cases/CasesMobile.vue` — 同上
...
<div class="mb-4">
  ...
  >
    <option :value="1">咨询阶段</option>
    <option :value="2">准备阶段</option>
    <option :value="3">一审阶段</option>
    <option :value="4">二审阶段</option>
    <option :value="99">结案</option>
    <option :value="999">归档</option>
  </select>
</div>
...
describe('ARCHIVED 状态只读守卫', () => {
  ...
  it('updateCaseService 对 ARCHIVED 案件抛出业务错误', async () => {
    ...
    prisma.cases.findUnique.mockResolvedValue({ id: 1, status: 999, deletedAt: null })

    await expect(
      updateCaseService(1, { title: '新标题' }, 42 /* userId */),
    ).rejects.toThrow(/归档|archived|只读/i)

    expect(prisma.cases.update).not.toHaveBeenCalled()
  })
  ...
  it('updateCaseService 对非 ARCHIVED 正常通过', async () => {
    ...
    prisma.cases.findUnique.mockResolvedValue({ id: 1, status: 1, deletedAt: null, userId: 42 })
    prisma.cases.update.mockResolvedValue({ id: 1, title: '新' })

    await expect(
      updateCaseService(1, { title: '新' }, 42),
...
打开 Step 1 找到的 `updateCaseService` 文件。在函数入口、加载案件记录之后、执行 update 之前，加：
...
export async function updateCaseService(
  ...
  const existing = await prisma.cases.findUnique({
    where: { id: caseId },
    select: { id: true, status: true, userId: true, deletedAt: true },
  })
  ...
  if (isCaseReadOnly(existing.status)) {
    throw new Error('案件已归档，不可编辑（如需恢复请联系管理员）')
  }

  return prisma.cases.update({ where: { id: caseId }, data: patch })
}
```

> 若现有函数签名不同，保留现有签名，只在"校验案件存在 + 用户权限"之后、执行 update 之前加 `isCaseReadOnly` 判断。
...
打开 `app/components/cases/CasesTable.vue`（或详情页的编辑按钮位置）：

```vue
<!-- 原按钮 -->
<Button @click="editCase(row.id)">编辑</Button>

<!-- 改为（加 disabled + tooltip） -->
<Button
  :disabled="isCaseReadOnly(row.status)"
  :title="isCaseReadOnly(row.status) ? '归档案件不可编辑' : ''"
  @click="editCase(row.id)"
>编辑</Button>
```
...
## ARCHIVED 只读
- [ ] 把一个测试案件 `UPDATE cases SET status=999 WHERE id=X` 改为归档
- [ ] 列表页编辑按钮灰色 + hover 提示"归档案件不可编辑"
...
├── CaseStatusBadge.vue                                [新建] 统一 6 档徽章
├── CasesTable.vue                                     [修改] 用 CaseStatusBadge + isCaseReadOnly
├── CasesGrid.vue                                      [修改] 同上
└── CasesMobile.vue                                    [修改] 同上

app/components/caseCreation/
└── ManualForm.vue                                     [修改] 平铺 5 字段 + 状态下拉 + getCurrentValues
...
- [ ] 所有数据库变更走 `bun run prisma:migrate`（本 plan 用 `--create-only` 后手工追加存量迁移 SQL）
- [ ] 存量 `status=2/3` 迁移到 `99`（CLOSED），非 `999`（ARCHIVED 仅通过未来手动归档产生）
- [ ] `updateCaseService` / `initAnalysisService` 都要 check `isCaseReadOnly`（前端灰化不够）
- [ ] `CaseStatusBadge` 用 shared types 的枚举/文案/色系（不在组件里硬编码）
- [ ] 图标一律 lucide（本 plan 未用图标，徽章纯色块 + 文字；后续如加图标必须 lucide）
