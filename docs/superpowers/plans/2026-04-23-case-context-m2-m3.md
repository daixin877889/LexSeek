...
- `server/services/memory/postProcess.ts` — 纯函数 mmr / versionScoring / recencyDecay
...
- `server/services/retrieval/types.ts` — `ALLOWED_TABLES` 加 `case_memories`
...
import { generateSummary } from '~~/server/services/ai/summaryService'
...
describe('generateSummary', () => {
  it('100 字场景（M2 材料）用 Haiku 4.5 + maxChars=100', async () => {
    ...
    const res = await generateSummary(mockModel as any, '一份很长的合同文本...', { maxChars: 100 })
  ...
  it('400 字场景（M4 分析）支持自定义 systemPrompt', async () => {
    ...
    const res = await generateSummary(mockModel as any, '风险评估分析正文...', {
      maxChars: 400,
  ...
  it('模型返回超长时截断到 maxChars', async () => {
    ...
    const res = await generateSummary(mockModel as any, 'text', { maxChars: 100 })
    expect(res.length).toBeLessThanOrEqual(100)
  })
})
```
...
- [ ] **Step 2: 扩展 `ChatModelConfig` 类型**

在 `chatModelFactory.ts` 顶部找到 `ChatModelConfig` interface，追加可选字段（**不改现有字段，保持向后兼容**）：
...
export interface ChatModelConfig {
  // ... 现有字段保留 ...
  /**
   * 可选：多段式 system prompt（支持 cache_control）。
   * 若提供，则优先于 `systemPrompt: string`（旧字段兼容保留）。
   */
  systemPromptSegments?: CachedPrompt
}
```

- [ ] **Step 3: Anthropic adapter 翻译 `CachedPrompt` 为 content block 数组**
...
vi.mock('~~/server/utils/prisma', () => ({
  default: {
    cases: { findUnique: vi.fn() },
    caseAnalyses: { findMany: vi.fn() },
  },
}))
vi.mock('~~/server/services/material/materialPipeline.service', () => ({
  getMaterialListWithSummaries: vi.fn().mockResolvedValue([
...
describe('buildContextSegments', () => {
  ...
  it('输出 4 段（角色/档案/摘要/动态），顺序正确', async () => {
    const prisma = (await import('~~/server/utils/prisma')).default as any
    prisma.cases.findUnique.mockResolvedValue({
      id: 1, title: '张李纠纷', courtName: '朝阳法院',
      plaintiff: ['张三'], defendant: ['李四'],
      summary: '房屋租赁纠纷', status: 3,
    })
    prisma.caseAnalyses.findMany.mockResolvedValue([
      { analysisType: 'risk_assessment', summary: '风险等级中高' },
    ])
    ...
    expect(segs.moduleSummaries).toContain('风险等级中高')
  ...
  it('无分析产物时 moduleSummaries 为空串', async () => {
    const prisma = (await import('~~/server/utils/prisma')).default as any
    prisma.cases.findUnique.mockResolvedValue({ id: 1, title: 'x' })
    prisma.caseAnalyses.findMany.mockResolvedValue([])
    const { buildContextSegments } = await import('~~/server/services/workflow/context/moduleContextBuilder')
...
import { getMaterialListWithSummaries } from '../../material/materialPipeline.service'
...
export async function buildContextSegments(params: Params): Promise<ContextSegments> {
  ...
  const [caseRecord, activeAnalyses, materials, memoryHits] = await Promise.all([
    ...
    getMaterialListWithSummaries(caseId).catch(() => []),
...
export function toCachedPrompt(segs: ContextSegments): CachedPrompt {
  ...
  if (segs.roleAndFlow) out.push({ text: segs.roleAndFlow })
...
```bash
git add server/services/workflow/context/moduleContextBuilder.ts tests/server/moduleContextBuilder.test.ts
...
export async function generateMaterialSummaryService(materialId: number): Promise<void> {
  try {
    ...
    const summary = await generateSummary(haiku, content, { maxChars: 100 })
 ...
 */
export async function getMaterialListWithSummaries(caseId: number): Promise<Array<{
  id: number
  name: string
  type: number
  summary: string | null
...
- [ ] **Step 1: 定位 saveCaseInfoService 调用点**

Run: `grep -n "saveCaseInfoService\|basic_info" server/services/case/*.ts`
Expected: 看到 `caseExtraction.service.ts` 里调 `saveCaseInfoService(caseId, text)` 之类。

- [ ] **Step 2: 注释调用点**

在 `caseExtraction.service.ts` 里找到调用位置，替换为：
...
// 存量数据保留不读，后续观察无引用再清理。
// 原调用：
// await saveCaseInfoService(caseId, formatCaseInfo(data))
```

`saveCaseInfoService` 函数本身不删除（可能被其它历史调用方用），只移除本处调用。

- [ ] **Step 3: 手工验证**

Run: `grep -n "saveCaseInfoService" server/ --include="*.ts" -r`
Expected: 只剩函数定义，没有活跃调用。
...
- [ ] **Step 2: 新增 case_memories**

编辑 `server/services/retrieval/types.ts`：

```ts
// 原：
// export const ALLOWED_TABLES = new Set<string>(['case_material_embeddings', 'law_embeddings'])
// 改为：
...
- [ ] **Step 3: 新建 shared/types/memory.ts**
...
```bash
git add server/services/retrieval/types.ts shared/types/memory.ts
git commit -m "feat(memory): 注册 case_memories 到 ALLOWED_TABLES + 新类型 shared/types/memory.ts

- ALLOWED_TABLES 加 case_memories，允许被 hybridSearchService 查询
...
export async function writeMemoryService(input: MemoryWriteInput): Promise<{ id: string }> {
  ...
  // 1. 版本链：同 subjectKey 的最新未失效记录 → 打失效
  if (input.subjectKey) {
...
import { recencyDecay, subjectVersionScoring, mmrFilter } from '~~/server/services/memory/postProcess'
...
describe('mmrFilter', () => {
  it('相似条目只选一条，覆盖不同方向', async () => {
    const hits = [
      { id: '1', text: '原告地址在北京', score: 0.9, metadata: {} as any, embedding: [1, 0, 0] },
      { id: '2', text: '原告住址为北京', score: 0.85, metadata: {} as any, embedding: [0.99, 0.01, 0] }, // 和 1 几乎重复
      { id: '3', text: '合同违约金 10%', score: 0.8, metadata: {} as any, embedding: [0, 1, 0] },
    ] as any
    const picked = await mmrFilter(hits, 'x', { lambda: 0.4, topK: 2 })
    expect(picked).toHaveLength(2)
    const picks = picked.map((p) => p.id).sort()
    expect(picks).toEqual(['1', '3'])  // 覆盖两个不同方向
  })
})
```
 ...
 * - lambda：0-1，0 纯多样性，1 纯相关性，推荐 0.4
 * - 依赖 hits 自带 embedding 向量做余弦相似度计算（若没有 embedding 回退为 score 排序）
 */
export async function mmrFilter(
  hits: Array<MemoryHit & { embedding?: number[] }>,
  _query: string,
  opts: { lambda: number; topK: number },
): Promise<MemoryHit[]> {
  const { lambda, topK } = opts
  if (hits.length <= topK) return hits

  // 检查是否所有 hits 都有 embedding
  const hasEmbeddings = hits.every((h) => Array.isArray(h.embedding))
  if (!hasEmbeddings) {
    // 回退：直接按 score 取 topK
  ...
  while (selected.length < topK && remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      for (const s of selected) {
        const sim = cosineSim(candidate.embedding!, s.embedding!)
...
Run: `npx vitest run tests/server/memory/postProcess.test.ts --reporter=verbose 2>&1 | tail -15`
Expected: 5 passed。
...
export interface RetrieveInput {
  tableName: string                                  // 'case_memories' | 'case_analysis_embeddings'
  query: string
  topK: number
...
/**
 * M3/M4 共享的召回公共入口：
 *   ① ②（复用 hybridSearchService）→ ③ pre-filter → ④ rerank → ⑤ MMR → ⑥ 版本链降权
 * 任一阶段失败都降级（rerank 不可达 → 跳过；embedding 缺失 → MMR 回退 topK）
 */
export async function retrieveWithReranking(input: RetrieveInput): Promise<MemoryHit[]> {
  ...
  const hybridHits = await hybridSearchService(tableName, query, 50, metadataFilter)

  // ③ Pre-filter
  const filtered = hybridHits.filter((h) => {
    if ((h.score ?? 0) < minScore) return false
    if (filterInvalidated) {
      const meta = h.metadata as CaseMemoryMetadata | undefined
  ...
  // ④ Rerank（降级：服务不可达 → 跳过，用 hybrid 分数）
  let reranked: Array<MemoryHit & { embedding?: number[] }>
  try {
    const rerankRes = await rerankDocuments(
      query,
      filtered.slice(0, 20).map((h) => ({ id: h.id, text: h.text })),
    )
    // 按 rerank 分数排序，保留原 metadata / embedding
    reranked = rerankRes.map((r) => {
      const orig = filtered.find((h) => h.id === r.id)!
      return { ...orig, score: r.score, metadata: orig.metadata as CaseMemoryMetadata }
    })
  } catch (e) {
    logger.warn('rerankerClient 不可达，降级走 hybrid 分数', { error: e })
    reranked = filtered.slice(0, 20).map((h) => ({ ...h, metadata: h.metadata as CaseMemoryMetadata }))
  }

  // ⑤ MMR 多样性（需 embedding；缺则回退）
  const diverse = await mmrFilter(reranked, query, { lambda: 0.4, topK: topK * 2 })

  // ⑥ 版本链降权（仅 M3 场景）
  const final = enableVersionScoring ? subjectVersionScoring(diverse) : diverse
...
Expected: 无错（`rerankerClient` 将在 Task 12 新建，暂用桩也可；为避免阻塞，可先 `import` 暂注释或 Task 12 完成再测试）。

> 若当前 import 'rerankerClient' 报错，先 stub：
> ```ts
> async function rerankDocuments(_q: string, docs: Array<{ id: string; text: string }>) {
>   return docs.map((d, i) => ({ id: d.id, score: 1 - i * 0.01 }))
> }
> ```
> Task 12 完成后删除 stub。
...
- postProcess: recencyDecay / subjectVersionScoring / mmrFilter 三个纯函数
- retrieveWithReranking: M3/M4 共享的召回入口，封装 hybrid + rerank + MMR + versionScoring
- 各阶段降级策略：rerank 不可达走 hybrid 分数；无 embedding MMR 回退 topK
spec §3.3"
```
...
// 辅助：加载最近消息（实际实现对接项目消息持久化层）
async function loadRecentAgentMessages(sessionId: string, limit: number): Promise<Array<{ role: string; content: string }>> {
  // 对接 LangGraph checkpoint 的 messages 通道，或项目现有 loadSubAgentThreads / getChatThreadState
  // 本 plan 的编码 Task 会用项目现有读消息 API 替换下面占位
  return []
}
...
- [ ] **Step 4: 注册到 CronScheduler**

`server/plugins/cron-scheduler.ts` 里应该有 `CronScheduler` 类。在该插件里（或新建的 `server/plugins/consolidator-worker.ts`）注册定时任务：

```ts
// server/plugins/consolidator-worker.ts（新建）
...
export default defineNitroPlugin(() => {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      logger.warn('consolidator worker tick 失败', { error: e })
    }
  }, INTERVAL_MS)
...
> 若项目 `CronScheduler` 是类而非 `setInterval` 直接用，改为复用该类实例：`new CronScheduler({ intervalMs: 10_000, lockTtlSeconds: 5, handler: async () => { ... } }).start()`。
...
```bash
git add server/services/memory/consolidator.service.ts server/plugins/consolidator-worker.ts tests/server/memory/consolidator.test.ts
...
server/plugins/
└── consolidator-worker.ts                              [新建] 每 10s 扫队列
