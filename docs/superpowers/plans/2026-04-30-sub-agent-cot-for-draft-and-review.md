      ...
      case 'sub_agent_tool_start': {
        ...
        if (d?.innerToolCallId && d?.toolName) {
          ...
          if (!exists) {
            ...
            const ai: any = new AIMessage({ content: '' })
            ai.id = d.cbRunId ?? d.innerToolCallId
            ai.tool_calls = [{ id: d.innerToolCallId, name: d.toolName, args: parsedArgs }]
...
跑完后检查 `prisma migrate status` 应显示 "up to date"。
...
```typescript
            ...
            // try { ... runAndDrainStream + 后续逻辑 } finally { 无论成功/失败都写 cotMessages }
            try {
                const drainResult = await runAndDrainStream(stream)
                if (!drainResult.success) {
                    // 跑失败也先写 cotMessages 让用户能查看失败前的进度
                    await updateContractReviewDAO(review.id, {
                        cotMessages: cotAccumulator as Prisma.InputJsonValue,
                    }).catch(err => logger.warn('write cotMessages on fail failed', { reviewId: review.id, err }))
                ...
                await updateContractReviewDAO(review.id, {
                    cotMessages: cotAccumulator as Prisma.InputJsonValue,
                }).catch(err => logger.warn('write cotMessages failed', { reviewId: review.id, err }))
            } catch (err) {
                // 兜底：异常路径也尽力写一次
