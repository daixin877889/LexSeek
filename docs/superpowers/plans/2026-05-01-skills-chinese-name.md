...
```bash
DATABASE_URL='postgresql://daixin:daixin88@localhost:5432/ls_new_testing?schema=public&TimeZone=UTC' bun run prisma:migrate
```

期望输出：`Database schema is up to date`（如果没有未应用迁移）或应用本次迁移到 ls_new_testing。
...
期望：全部 PASS（含 Task 2 / 3 已写的 case）。如果有"文件系统删除被 markSkillsDisabledByNamesDAO 标 DISABLED"的回归测试 fail，说明 fix 误伤——回头检查（spec §同步逻辑改动 表格已声明该路径仍由 markSkillsDisabledByNamesDAO 单独负责）。
...
export async function updateSkillCustomTitleService(name: string, raw: string | null) {
    ...
    const row = await updateSkillCustomTitleDAO(name, customTitle)
    // 用户端映射表的下游缓存：清节点配置 + filesystem backend 缓存
...
新建 `tests/server/admin/skills/update.api.test.ts`：
...
describe('PATCH /api/v1/admin/skills/:name', () => {
    ...
    beforeAll(async () => {
        // 用项目测试账号 13064768490 / daixin88 登录拿 token
        const r = await $fetch<{ data: { token: string } }>('/api/v1/auth/login', {
            method: 'POST',
            body: { phone: '13064768490', password: 'daixin88' },
        })
    ...
    it('设置 customTitle', async () => {
        ...
        const r = await $fetch<{ code: number }>(`/api/v1/admin/skills/${name}`, {
            method: 'PATCH',
            headers: { Authorization: `Bearer ${adminToken}` },
            body: { customTitle: '我的中文名' },
        })
    ...
    it('超长（>200）报 400', async () => {
        ...
        const r = await $fetch<{ code: number }>(`/api/v1/admin/skills/${name}`, {
            method: 'PATCH',
            headers: { Authorization: `Bearer ${adminToken}` },
            body: { customTitle: 'x'.repeat(201) },
        })
        expect(r.code).toBe(400)
    })

    it('skill 不存在报 404', async () => {
        const r = await $fetch<{ code: number }>(`/api/v1/admin/skills/__not_exist__`, {
            method: 'PATCH',
            headers: { Authorization: `Bearer ${adminToken}` },
            body: { customTitle: 'x' },
        })
...
> 注意：项目当前 admin 接口走 RBAC 中间件，测试账号是 super_admin，应该直接通过；如果测试 fail 在 403，需要在 `server/api/v1/admin/skills/[name].patch.ts` 注册到 RBAC 权限表（见 Task 14），先把 handler 写完跑测试看具体错。
...
新建 `tests/server/skills/labels.api.test.ts`：
...
describe('GET /api/v1/skills/labels', () => {
    ...
    it('未登录返回 401', async () => {
        const r = await $fetch<{ code: number }>('/api/v1/skills/labels')
        expect(r.code).toBe(401)
    })

    it('登录后返回启用 skill 的 name→label 映射', async () => {
        ...
        const r = await $fetch<{ data: Array<{ name: string; label: string }> }>(
            '/api/v1/skills/labels',
            { headers: { Authorization: `Bearer ${userToken}` } },
        )
        const map = Object.fromEntries(r.data.map(x => [x.name, x.label]))
...
WriteSkillFileTool 当前模板里 `:title="dirPath"` 保持不变（鼠标悬停仍看真实英文路径）。模板内副标题段没有显式渲染 dirPath（仅作 `:title`），所以**仅 script 加 dirDisplay 但模板不变**——目前 dirPath 在该组件里只用作 hover title，本次不动这行。

> 备注：如果未来 WriteSkillFileTool 副标题里也想显式展示中文路径，把 `<p ... :title="dirPath">` 内的副标题文字加一段 `<template v-if="dirDisplay"> · {{ dirDisplay }}</template>` 即可。本期不扩。
...
> **注**：上一步说"模板不变"，但 16.3 仍要 commit 一份 dirDisplay computed 的改动，便于未来扩展。如果觉得"现在不展示就别加"，可以省略 16.3 整个任务（属于技术减项，不影响用户能感知的中文显示）。
...
git commit -m "$(cat <<'EOF'
feat(ui): WriteSkillFileTool 加 dirDisplay computed 备未来路径中文化
EOF
)"
```
