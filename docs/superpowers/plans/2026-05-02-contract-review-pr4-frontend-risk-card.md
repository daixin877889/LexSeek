...
- **PR 3（路线 2 锚点解析）已完整合并**（10 commits, e0d0d47f~f3e402f2）——后端 `persistAiRisksAsContractRows` 已经真实把 quote 字段写进 DB（contractRisk.service.ts:134-137 用 `resolveQuoteAnchor` 三档 fallback）；PR 4 上线即可立即生效 Layout A 的"问题片段"框 + 完整原文 quote 字符级高亮
- 工作区基于 `dev`（PR 2/3 已合）起独立 worktree（superpowers:using-git-worktrees）

**PR 2/3 真实落地状态（PR 4 plan 已对齐）**：
- `ContractRiskEntity`（shared/types/contract.ts:478-518）已含全部双锚点字段
- **`Risk` interface（shared/types/contract.ts:148-169）PR 3 已加 `problemSentenceIds?: number[]` + `problematicQuote?: string`**——为了承载 LLM 输出在 service 层流转；UI 不读 `problemSentenceIds`，但 `problematicQuote` UI 要用，**已通过继承 Risk 自动到达 RiskDisplayPhaseB**，PR 4 Task 1 不能再重复声明（TS 子接口不能放宽父字段类型 `string | undefined` → `string | null | undefined`）
- `RiskDisplayPhaseB`（shared/types/contract.ts:186-190）当前**含** `problematicQuote`（继承 Risk）但**未含** `quoteCharStart` / `quoteCharEnd`，由 PR 4 Task 1 仅扩这 2 字段（不重复加 problematicQuote）
- `PATCH /risks/:riskId` 已加 `bodySchema.strict()`（PR 3 commit f3e402f2，spec § 5.0）—— PR 4 不动 PATCH
...
- 前端目前**没有任何文件**读 quote 字段（grep 全 0），PR 4 是首消费者
...
### 类型扩展
- 修改：`shared/types/contract.ts`（`RiskDisplayPhaseB` 加 **2 字段** `quoteCharStart` / `quoteCharEnd`；`problematicQuote` 已被 PR 3 commit d25a3f38 加到父类型 `Risk`，**自动继承不重复声明**；**不加** `quoteMatchSource`——前端 UI 不读，运维 SQL 直查 `contract_risks.quote_match_source`，YAGNI）
...
| 类别 | 字段 / 标识 | 类型 | 来源 / 写入路径 |
|---|---|---|---|
| 已存在（PR3 加，继承） | `Risk.problematicQuote` | `string \| undefined` | PR 3 commit d25a3f38 已加；mapper 透传 `e.problematicQuote ?? undefined` 适配 |
| 类型扩展 | `RiskDisplayPhaseB.quoteCharStart` | `number \| null \| undefined` | `mapEntityToDisplay(e)` 透传 `e.quoteCharStart ?? null` |
...
## Task 1：扩展 RiskDisplayPhaseB 类型加 quote charStart/End

**Files:**
- Modify: `shared/types/contract.ts:186-190`

`RiskDisplayPhaseB` 当前只在 `RiskDisplay` 上加了 `source` / `orphaned` / `originalClauseText` 三字段。**`problematicQuote` 已被 PR 3 commit d25a3f38 加到父类型 `Risk`（line 165-167），自动继承不重复声明**——TS 子接口不能放宽父字段类型（`string | undefined` → `string | null | undefined` 会报错）。本任务只加 PR 3 没加进 Risk 的 2 个字段：`quoteCharStart` / `quoteCharEnd`，让 Layout A 的"完整原文"块能做 quote 字符级高亮。

> **YAGNI 1**：spec § 4.4 列了 4 个 quote 字段，但 `quoteMatchSource` 前端 UI 不读，运维监控走 spec § 10.3 SQL 直查 DB 列。**PR 4 类型扩展只加 2 个字段**。

> **YAGNI 2**：`problematicQuote` 已通过 PR 3 加到 Risk 接口（PR 3 用它在 LLM → service → DB 流转）；PR 4 UI 通过 `Risk` 父类型继承自动得到该字段，不需重复声明。

> **PATCH 安全**：`PATCH /risks/:riskId` 已由 PR 3 commit f3e402f2 加 `.strict()`——任何带 quote/clause 字段的 body 直接 400 拒绝（spec § 5.0）。Risk 接口含 `problematicQuote` 仅为内部数据契约，不影响 PATCH 行为。
 ...
 * 前端依据这些字段做"外部新增分组""孤立批注区"等分组渲染。
 *
 * PR 4：新增 quoteCharStart / quoteCharEnd 两字段（problematicQuote 已被 PR 3 加到父类型 Risk，
 * 通过继承自动可用）。由 ContractReviewPanel.mapEntityToDisplay 从 ContractRiskEntity 透传，
 * 给 Layout A 的完整原文 quote 字符级高亮使用；PATCH risks 接口已 .strict() 拒绝（spec § 5.0）。
 * 运维埋点用 quoteMatchSource SQL 直查 DB 列，前端 UI 不读不传。
 */
export type RiskDisplayPhaseB = RiskDisplay & {
    source?: RiskSource
    orphaned?: boolean
...
const effectiveRisks = computed<RiskDisplayPhaseB[]>(() => {
     ...
     * 触发 dmp.diff_main(undefined) Throw 让整个 Vue 渲染崩溃 + 风险卡无法点击。
     *
     * PR 4：把 problematicQuote / quoteCharStart / quoteCharEnd 三个 quote 锚点字段
     * 透传到 RiskDisplayPhaseB，给 Layout A 的"问题片段"块 + 完整原文 quote 字符级高亮用。
     * PR 3 已合，DB 已有真实 quote 数据；PR 4 上线即生效。
     * problematicQuote 已被 PR 3 加到 Risk 父接口，此处 mapper 透传补完接口契约。
    ...
    function mapEntityToDisplay(e: any): RiskDisplayPhaseB {
        return {
            ...
            matchedPointCode: e.code ?? undefined,
            archivedStatus: e.archivedStatus,
            // PR 4：双锚点 quote 字段透传（PR 3 已落库真实值；缺失走 null/undefined 降级）
            // problematicQuote 用 ?? undefined：Risk 父类型上是 string | undefined（PR 3 commit d25a3f38 加），
            //   不能用 ?? null（会触发 "null 不能赋给 string | undefined" 类型错）
            // quoteCharStart/End 用 ?? null：RiskDisplayPhaseB 上是 number | null | undefined，兼容
...
## Task 2.6：RiskEditDialog.handleConfirm 保留 PR3 加的 LLM 字段

**Files:**
- Modify: `app/components/assistant/contract/RiskEditDialog.vue:65-82`

**为什么 PR4 要做这个**：PR3 commit d25a3f38 在 `Risk` 接口加了 `problemSentenceIds?: number[]` + `problematicQuote?: string`（LLM 输出，承载 quote 锚点解析的中间值）。但 `RiskEditDialog.handleConfirm` 用**手写字段列表**构造新 Risk（11 个字段：id/clauseIndex/clauseText/level/category/problem/legalBasis/analysis/risk/suggestion/suggestedClauseText），**漏传** PR3 新加的 2 字段。后果：

1. 用户在 UI 编辑任何风险条目（哪怕只改一个文字），保存后 emit 出去的 Risk 对象 problematicQuote 变成 undefined
2. PR3 努力建立的精确 quote 锚点在用户首次编辑后**永久丢失**
3. **PR4 自己想消费的 problematicQuote 字段被清空** → Layout A 的"问题片段"块永久不再渲染（spec § 6.4 quote=null 降级路径）→ PR4 的核心视觉效果脆弱

PR4 是首个真正消费 problematicQuote 的 PR，**有责任顺手补完整 PR3 在编辑路径的字段保留**。

> **不修法的范围**：用户**主动改业务文字**（problem / suggestion / level 等）后，原 problematicQuote / problemSentenceIds 仍**保留原值**——这是合理的，因为锚点跟原条款挂钩，业务字段编辑不应该破坏与原文的锚定（spec § 5.0 同精神）。如果未来要支持"重定位锚点"这是 v2 功能。

- [ ] **Step 2.6.1：编辑 handleConfirm 加 spread 原 risk + 显式覆盖 form 字段**

打开 `app/components/assistant/contract/RiskEditDialog.vue`，定位第 65-82 行：
...
改为（spread 原 risk → 显式覆盖 form 字段，新增字段自动透传）：
...
function handleConfirm() {
    ...
    const f = form.value
    emit('confirm', {
        // PR 4：spread 原 risk 保留 PR 3 新加的 problemSentenceIds / problematicQuote
        // 以及 matchedPointCode / clauseParagraphIndex 等未感知字段，避免编辑后清空 quote 锚点。
        // 业务字段（problem / suggestion / level 等）由下方 form 字段显式覆盖。
        ...(props.risk ?? {}),
        id: props.risk?.id ?? crypto.randomUUID(),
        clauseIndex: typeof f.clauseIndex === 'number' ? f.clauseIndex : Number(f.clauseIndex),
        clauseText: f.clauseText,
...
> **关键点**：`...(props.risk ?? {})` 在 form 字段**之前** spread——form 字段会覆盖原 risk 的同名业务字段；只有原 risk 上 form 没覆盖的字段（problemSentenceIds / problematicQuote / matchedPointCode 等）才被保留。

- [ ] **Step 2.6.2：补一个测试用例验证字段保留**

打开 `tests/app/components/assistant/contract/RiskEditDialog.test.ts`（如果不存在则跳过本步），在末尾加：

```typescript
describe('RiskEditDialog · 编辑保留 LLM 字段（PR 4）', () => {
    it('编辑业务字段后 emit 的 Risk 仍含 problematicQuote / problemSentenceIds', async () => {
        const original: Risk = {
            id: 'r1',
            clauseIndex: 3,
            clauseText: '原条款',
            ...
            matchedPointCode: 'payment_default',
        }
        const w = mount(RiskEditDialog, { props: { open: true, risk: original } })
        // 模拟用户改 problem 字段
        // ...（按既有测试 helper 触发表单更新 + 点确认）
        const emitted = w.emitted('confirm')?.[0]?.[0] as Risk
        expect(emitted.problematicQuote).toBe('0.05%')
        expect(emitted.problemSentenceIds).toEqual([1, 2])
        expect(emitted.matchedPointCode).toBe('payment_default')
    })
})
```

> 如果既有 `RiskEditDialog.test.ts` 文件不存在或测试结构差异大，本步可跳过——主测试覆盖在 Task 7 完成；本用例属保险断言，缺失不阻断。

- [ ] **Step 2.6.3：跑既有 RiskEditDialog 测试 + Task 7 涉及的测试 sanity check**

```bash
npx vitest run tests/app/components/assistant/contract/RiskEditDialog.test.ts tests/app/components/assistant/contract/RiskListPanel.test.ts --reporter=verbose 2>&1 | tail -30
```

Expected: 全部 PASS（spread 原 risk 不破坏既有断言——既有 emit 断言只验业务字段，新增字段透传不冲突）。

- [ ] **Step 2.6.4：commit**

```bash
git add app/components/assistant/contract/RiskEditDialog.vue \
        tests/app/components/assistant/contract/RiskEditDialog.test.ts
git commit -m "fix(contract): RiskEditDialog 保留 LLM 字段防 quote 锚点丢失"
```
...
stubs 表（line 144-160）也无需追加 `Switch` —— 该组件目前作为真实 shadcn 渲染，既有"隐藏已处置"测试已经通过，不要替换。

**Edit 指令（精确）**：在 `tests/app/components/assistant/contract/RiskListPanel.test.ts:144-160` 的 `const stubs = {` 内、`AlertDialogCancel: alertBtn('AlertDialogCancel'),`（line 159）那一行**之后、闭合 `}` 之前**插入下面 3 个 key（**裸 key 列表，不要再写 `const stubs = { ... }` 包裹，避免被误读为整体替换原有 14 个 stub**）：

```typescript
    // 在既有 stubs 对象闭合 } 之前插入（不是替换整个 stubs，是追加 3 个 key）
...
- `RiskDisplayPhaseB` 扩展 `quoteCharStart` / `quoteCharEnd` 两字段（`problematicQuote` 已被 PR 3 加到父类型 `Risk` 自动继承；`quoteMatchSource` 不前端透传，YAGNI）
- `ContractReviewPanel.mapEntityToDisplay` 把 `ContractRiskEntity` 上的 quote 字段透传到 RiskDisplayPhaseB
- `RiskEditDialog.handleConfirm` 改为 spread 原 risk + 显式覆盖 form 字段，防 PR 3 加的 LLM 字段（problematicQuote / problemSentenceIds）在用户编辑后被清空
