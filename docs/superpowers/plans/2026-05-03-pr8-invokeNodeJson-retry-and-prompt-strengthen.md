...
**Why a script not raw SQL**：v3 content 可能含 `$content$` 字符串破坏 dollar-quoted、含单引号破坏字面量；用 prisma 客户端处理转义最稳妥。脚本独立放 `server/scripts/` 与既有 `importContractPlaybooks.ts` 同模式。

**Why 调既有 service 而不直接 prisma**：项目已有 `server/services/node/prompt.service.ts` 封装：
- `updatePromptService(id, { content })` 自动生成 `vN+1` 版本号（`generateNextVersion`，避免脚本写死 'v4' 在 dev 库已有 v4/v5 时冲突）+ 自动提取 `{{var}}` 模板变量
- `activatePromptService(id)` 事务原子切换同节点同 type 的 status（已封装）

直接 prisma 操作是反模式：违反 service-DAO 分层规则、重复实现版本号生成 / 事务切换逻辑。

- [ ] **Step 1：写脚本 `server/scripts/upgradeContractAnalyzePromptV4.ts`**

```typescript
/**
 * PR8 Task 3：把 contractReviewAnalyzeClause prompt 升级一个版本（v3 → v4，或当前
 * active version → 自动生成的下一版本号），追加 \n 错误反例段（spec §4.2）。
 *
 * 操作（全部走既有 prompt.service，不直接操作 prisma 表）：
 *   1. prisma 查当前 active prompt 拿 id
 *   2. updatePromptService(id, { content }) → 自动生成 vN+1 + 自动提取变量 + INSERT 新版本（status=0）
 *   3. activatePromptService(newId) → service 内事务原子切换同节点同 type 的 status
 *
 * 跑法：bunx tsx server/scripts/upgradeContractAnalyzePromptV4.ts
 * 幂等：内容比对 — 如果 active prompt 内容已含 V4_APPEND 段说明已升级过，跳过
 */
import { prisma } from '~~/server/utils/db'
import { updatePromptService, activatePromptService } from '~~/server/services/node/prompt.service'

const PROMPT_NAME = 'contractReviewAnalyzeClause'

// spec §4.2 追加段（保持与 plan / spec 文本完全一致；幂等检测用首行 marker）
const V4_APPEND_MARKER = '## suggestedClauseText 输出格式约束（铁律）'
const V4_APPEND = `
...
async function main(): Promise<void> {
    // 1. 找当前 active prompt（应为 v3，但兼容更新版本）
    const active = await prisma.prompts.findFirst({
        where: { name: PROMPT_NAME, status: 1 },
    })
    if (!active) {
        throw new Error(`[upgrade-prompt-v4] 找不到 ${PROMPT_NAME} status=1 源版本`)
    }
    console.log(`[upgrade-prompt-v4] 基线版本: ${active.version} (id=${active.id}, content_len=${active.content.length})`)

    // 2. 幂等保护：active 内容已含 V4_APPEND 段说明已升级过
    if (active.content.includes(V4_APPEND_MARKER)) {
        console.log(`[upgrade-prompt-v4] active prompt 已含升级段，无需再升级`)
        return
    }

    // 3. 通过 service 创建新版本（service 自动 vN+1 + 提取变量 + INSERT status=0）
    const newPrompt = await updatePromptService(active.id, {
        content: active.content + V4_APPEND,
    })
    console.log(`[upgrade-prompt-v4] 创建新版本: ${newPrompt.version} (id=${newPrompt.id}, content_len=${newPrompt.content.length})`)

    // 4. 通过 service 激活新版本（service 内事务原子切换 status）
    await activatePromptService(newPrompt.id)
    console.log(`[upgrade-prompt-v4] 已启用 ${newPrompt.version}，原 ${active.version} (id=${active.id}) status=0`)
...
预期输出（version 号由 `generateNextVersion` 自动生成，通常是 v4，dev 库已有更多版本时可能是 v5/v6）：

```
[upgrade-prompt-v4] 基线版本: v3 (id=<n>, content_len=<m>)
[upgrade-prompt-v4] 创建新版本: v4 (id=<n+1>, content_len=<m+~1100>)
[upgrade-prompt-v4] 已启用 v4，原 v3 (id=<n>) status=0
...
预期输出（active 内容含 V4_APPEND_MARKER → 跳过升级）：

```
[upgrade-prompt-v4] 基线版本: v4 (id=<n+1>, content_len=<m+~1100>)
[upgrade-prompt-v4] active prompt 已含升级段，无需再升级
```
...
+ 3 次重试三层防御。脚本通过既有 prompt.service 调 updatePromptService 自动
生成 vN+1 + activatePromptService 事务原子切换 status，不直接操作 prisma 表
（避免重复实现 service 层已封装的版本号生成 / 事务切换逻辑）。

幂等：active prompt 内容已含 V4_APPEND_MARKER 时跳过升级。
