...
- `extractTextFromAsrResult`（纯文本，pipeline）和 `extractTextFromSimplifiedResult`（带说话人/时间戳，asr）**职责互补，全部保留**——前者用于摘要 LLM 输入和文本渲染，后者用于 RAG embedding ✓
...
- Task 5：识别完成回调追加摘要触发（OssFile 级 fire-and-forget）
- Task 6：识别状态接口 `recognized` 判定升级
- Task 7：`runRecognitionAndEmbeddingPipeline` 阶段 3 摘要 + 终态轮询 + SSE 进度（含 PREPARE_MATERIALS 类型）
...
- Modify: `prisma/models/recognition.prisma`（更新 4 张表 `summary` 字段注释统一为"200 字摘要"）
...
**注意：保留 `UpdateAsrRecordInput.summary` / `updateAsrRecordService` 入参类型字段**——Task 3 的摘要写入还要用。只删 `embedAsrRecordService` 内 `summary: text` 这一处实际写入。
 ...
 * 与 extractTextFromSimplifiedResult 不同：本函数不带说话人/时间戳，纯文本；用于摘要 LLM 输入和材料正文读取。
 */
...
改为（用纯文本 `extractTextFromAsrResult` —— 摘要 LLM 输入用）：
...
**注意**：识别完成时 caseMaterials 行可能尚未创建（用户还在 AiPromptInput 阶段），此场景下 ossFile 级摘要在用户提交分析后由 ensureMaterialsReadyService 内部触发（Task 7）。本 Task 只触发已有 caseMaterials 的摘要。
...
git commit -m "feat(cases): 识别完成后 fire-and-forget 触发摘要生成
...
git commit -m "feat(api): 状态接口 recognized 升级为'识别+摘要双就绪'

让 AiPromptInput 发送按钮自动等到摘要生成完成后才放行（前端零改动）。
mineruTask 命中时强制 recognized=false（mineruTask 表无 summary 字段）。"
```

---

### Task 7：runRecognitionAndEmbeddingPipeline 阶段 3 摘要 + 终态轮询 + SSE 进度
...
```typescript
     ...
     *
     * 由 caseProcessMaterialMiddleware 在等待识别+摘要双就绪期间发出 phase:'start'/'progress'/'end'。
...
async function runRecognitionAndEmbeddingPipeline(
    ...
    // 阶段 3：对所有已识别但 summary 缺失的材料并行触发摘要生成
 ...
 * 材料就绪保底中间件
 *
 * 升级版：等到所有材料"识别 + 200 字摘要"双就绪才放行 Agent。
...
git commit -m "feat(cases): 中间件升级'识别+摘要双就绪'判定 + SSE 进度推送

- ensureMaterialsReadyService 加阶段 3：并行触发已识别材料的摘要生成
