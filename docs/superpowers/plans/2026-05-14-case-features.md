...
function handleChange(val: string | string[]) {
  ...
  emit('update:modelValue', val as CaseStance)
}
</script>
...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        await updateCaseService(caseId, updates as any)
...
git commit -m "feat(case-detail): 基础信息卡片补全 7+1 字段展示"
```
...
git commit -m "feat(case-detail): 编辑信息表单扩展至基础信息全字段 + stance"
```
...
`tests/server/case/initSessionsList.test.ts`：
...
let userId: number, caseId: number
...
beforeAll(async () => {
  ...
  caseId = c.id
  // 2 个 type=2 session
  await prisma.caseSessions.create({ data: { sessionId: 'sid-init-1', scope: 'case', caseId, userId, type: 2, title: '首次批量分析' } })
  await prisma.caseSessions.create({ data: { sessionId: 'sid-init-2', scope: 'case', caseId, userId, type: 2, title: null } })
  // 1 个 type=3 session（不应进列表）
  await prisma.caseSessions.create({ data: { sessionId: 'sid-mod-1', scope: 'case', caseId, userId, type: 3, title: '模块对话' } })
})

describe('GET /api/v1/cases/analysis/init-sessions', () => {
  it('返回该案件所有 type=2 会话，按 updatedAt 倒序，且不含 type=3', async () => {
    const res: any = await $fetch(`/api/v1/cases/analysis/init-sessions?caseId=${caseId}`, {
      headers: { cookie: `lexseek_user=${userId}` /* 由测试基建注入 */ },
    })
...
> 测试基建注入 cookie 的方式见 `tests/_infra/` 的 README；如果项目用 nitro 内置 auth mock 则用对应 helper。
...
export default defineEventHandler(async (event) => {
    ...
    const result = sessions.map((s, idx) => ({
        ...
        title: s.title ?? `批量分析 #${sessions.length - idx}`,
...
> 如果 `listSessionsWithActiveRunDAO` 已经按 updatedAt 排序，按上面 `idx` 倒推序号即可（最新一条标 #N，最早一条标 #1）。若 DAO 不排序，先 sort。
...
git commit -m "feat(case): 新增 BatchAnalysisPopover 批量分析下拉组件"
```
...
git commit -m "feat(case): 详情页批量分析按钮改为下拉浮层"
```
...
git commit -m "feat(case-detail): 新建文书跳转 URL 增加 autoAi=1 启动标记"
```
...
if (route.query.autoAi === '1') {
  // 等草稿数据 + useDocumentAgent() 就绪后执行
  watch(
    () => [draft.value, agentOpen.value],
    ([d]) => {
      if (!d) return
      const stop = () => {} // watch 自动 stop on unmount
      const templateName = (d as any).templateName ?? '本文书'
      openAgent()
      nextTick(() => {
        handleChatSubmit({ text: `请根据当前案件信息生成《${templateName}》` })
        // 清除 query 防止刷新重复触发
        const { autoAi, ...rest } = route.query
...
# 若有杂项修补
git add <files>
git commit -m "chore(case): 案件功能迭代落地后的 lint/typecheck 修整"
```
