...
**Architecture:** 导出端给修订标记写一份 customXml 身份证（`redlineRefs.xml`，镜像现有批注身份证机制）；回传端新增解析器 + 双层判定纯函数（w:id 精确层 + 正文比对兜底层）识别处置，叠加进现有 6 步上传链路；防误删保护从「批注命中率」升级为「批注+修订统一覆盖率」。署名是一个用户字段，导出时替换 AI 修订/批注的作者名。
...
1. **`customXmlRegistrar` 仅供 redlineInjector 使用**：spec §5.3 提议 redlineRefs 与 annotationRefs 共用注册器。实施时不重构 `commentInjector` 的 `ensureContentTypesRegistered`/`ensureDocumentRelsRegistered`——它们稳定、有测试、且耦合处理 comments+annotationRefs，为边际 DRY 重构会牵动 `@deprecated injectComments` 与既有测试。新注册器消除 redlineInjector 的新增重复即可。此为技术取舍，不影响架构与功能。
2. **正文比对用全文语料**：spec §6.2 提到「优先定位段落、再在段落内比对」。本计划实现「全文 corpusT/corpusDel」基线（spec 的「定位失败退回全文」分支）。`classifyRedlineDecision` 入参收 `corpusT`/`corpusDel` 两个字符串，未来若要段落级精度，调用方传入段落文本即可，无需改函数。
...
- `server/agents/contract/docx/redlineParser.ts` — 解析 `redlineRefs.xml` + 存活 ins/del id + 正文语料；导出双层判定纯函数 `classifyRedlineDecision` 与类型 `RedlineRefEntry`。
...
- `server/agents/contract/docx/redlineInjector.ts` — 加署名 author + 写 redlineRefs.xml。
...
- `docs/tech-docs/backend/contract.md` — 文档同步。
...
新建 `tests/server/users/contractSignature.service.test.ts`（目录不存在则一并创建）：
...
async function createUser(name: string, signature: string | null) {
    return prisma.users.create({
        data: {
            ...
            contractExportSignature: signature,
        },
    })
}
...
- [ ] **Step 3: `profile.put.ts` 接受并写入新字段**
...
```bash
git add server/services/users/userResponse.service.ts server/api/v1/users/me.get.ts server/api/v1/users/profile.put.ts app/store/user.ts app/pages/dashboard/settings/profile.vue
...
在 `tests/server/assistant/contract/docx/commentInjector.annotations.test.ts` 追加用例（参照该文件现有构造 docx fixture 与读取 `word/comments.xml` 的方式）：

```typescript
it('signature 选项：AI 批注作者用署名、律师批注去 LS: 前缀', async () => {
    // annotations 含一条 ai、一条 lawyer（authorName='陈律师'）
    const result = await injectAnnotations(baseDocxBuffer, annotations, REVIEW_ID, { signature: '王明远' })
    const commentsXml = await readCommentsXml(result.buffer)
...
(c) `buildCommentsXmlFromAnnotations` 增加 `signature` 形参，作者名按 authorType 计算、不再加 `LS:`：
```typescript
...
(d) `injectAnnotations` 内调用处传署名（解析一次 `const signature = opts?.signature?.trim() || 'AI'`，传给 `buildCommentsXmlFromAnnotations`）：
```typescript
    const signature = opts?.signature?.trim() || 'AI'
    // ...
    writeTextToZip(zip, 'word/comments.xml',
        buildCommentsXmlFromAnnotations(annotations, refsByAnnotationId, wordIdByAnnotationId, reviewId, signature),
    )
```

(e) 删除 `buildAuthorField` 的 import（`import { generateWordCommentRef, buildAuthorField }` → `import { generateWordCommentRef }`）。

- [ ] **Step 3: 移除无用的 `buildAuthorField`**

Run: `grep -rn "buildAuthorField" server/ tests/`
Expected: 仅 `wordCommentRef.ts` 自身定义处出现（commentInjector 已移除引用）。

`server/agents/contract/utils/wordCommentRef.ts`：删除 `buildAuthorField` 函数（含其 JSDoc 注释块）。

- [ ] **Step 4: `redlineInjector` 支持署名**

`server/agents/contract/docx/redlineInjector.ts`：

(a) `injectRedlineMarks` 的 `options` 形参类型加**可选** `signature`（设可选避免既有调用方/测试编译失败；编排函数总会显式传入真实署名）：
```typescript
    options: { reviewId: number, idStart: number, signature?: string },
```
函数体开头解析作者名（不传时回退中性默认值）：
```typescript
    const redlineAuthor = options.signature?.trim() || '审查人'
```

(b) `applyRedlineToParagraph` 的 input 对象加 `author: string` 字段；其内部两处 `makeElement('w:del', { ..., 'w:author': REDLINE_AUTHOR, ... })` 与 `buildInsertNode(...)` 调用，把 `REDLINE_AUTHOR` 换成 `input.author`。`buildInsertNode` 的 input 对象同样加 `author: string`，其 `makeElement('w:ins', { ..., 'w:author': REDLINE_AUTHOR, ... })` 换成 `input.author`。
...
Run: `npx vitest run tests/server/assistant/contract/docx/commentInjector.annotations.test.ts tests/server/assistant/contract/docx/commentInjector.test.ts tests/server/assistant/contract/docx/redlineInjector.test.ts --reporter=verbose`
Expected: PASS。`signature` 为可选参数，既有调用不传不会编译失败；但既有用例若因作者名变化断言失败，按新预期更新断言：批注作者去 `LS:` 前缀（`LS:AI`→署名或回退 `AI`、`LS:{律师名}`→律师名）；修订标记 `w:author` 由 `LexSeek AI` 改为署名（未传 `signature` 的既有用例回退值为 `审查人`）。
...
```bash
git add server/agents/contract/docx/redlineInjector.ts server/agents/contract/docx/commentInjector.ts server/agents/contract/utils/wordCommentRef.ts server/agents/contract/contractReviewRebuild.service.ts server/agents/contract/contractReviewVersion.service.ts tests/server/assistant/contract/docx/commentInjector.annotations.test.ts
...
在 `tests/server/assistant/contract/docx/redlineInjector.test.ts` 追加用例（沿用该文件构造 `RedlineRisk[]` 与 docx fixture 的方式）：

```typescript
it('导出后写入 redlineRefs.xml 身份证', async () => {
    const result = await injectRedlineMarks(baseDocxBuffer, risks, { reviewId: 871, idStart: 100, signature: '王明远' })
    ...
    // 每条成功装填的 risk 应有一个 ref（spansByRiskId 的 key 即 riskId）
...
(a) import 加：
```typescript
import { makeLeaf, makeXmlDecl } from './xmlAst'   // 合并进现有 xmlAst import
...
**Files:**
- Create: `server/agents/contract/docx/redlineParser.ts`（本任务先建文件 + 判定函数；Task 9 补解析函数）
...
const ref: RedlineRefEntry = { riskId: 1, delIds: [10], insId: 11 }
...
新建 `server/agents/contract/docx/redlineParser.ts`（Task 9 将向同文件追加 `parseRedlineMarks`）：

```typescript
/**
 * 修订标记回传解析与处置判定（spec §6）。
 *
 * - parseRedlineMarks（Task 9）：从回传 docx 读 redlineRefs.xml + 存活 ins/del id + 正文语料。
...
export interface ClassifyRedlineInput {
    ...
    /** 回传文档全部 <w:t> 文本，已 normalizeForMatch 归一化 */
    corpusT: string
    /** 回传文档全部 <w:delText> 文本，已 normalizeForMatch 归一化 */
    corpusDel: string
...
## Task 9: 修订解析器 parseRedlineMarks
...
新建 `tests/server/assistant/contract/docx/redlineParser.test.ts`（沿用 `redlineInjector.test.ts` 的 fixture 构造方式产出含修订标记的 docx）：

```typescript
import { describe, it, expect } from 'vitest'
import { injectRedlineMarks } from '~~/server/agents/contract/docx/redlineInjector'
import { parseRedlineMarks } from '~~/server/agents/contract/docx/redlineParser'

describe('parseRedlineMarks', () => {
    it('解析 injectRedlineMarks 产出的 docx：refs + reviewId + 存活 id', async () => {
        const result = await injectRedlineMarks(baseDocxBuffer, risks, { reviewId: 871, idStart: 100, signature: '王明远' })
        const parsed = await parseRedlineMarks(result.buffer)
        expect(parsed.reviewId).toBe(871)
        expect(parsed.refs.length).toBe(result.spansByRiskId.size)
        ...
        expect(parsed.corpusT.length).toBeGreaterThan(0)
    })

    it('无 redlineRefs.xml 的 docx：reviewId=null、refs 为空', async () => {
        const parsed = await parseRedlineMarks(baseDocxBuffer)
...
Expected: FAIL（`parseRedlineMarks` 未导出）。

- [ ] **Step 2: 实现 parseRedlineMarks**

`server/agents/contract/docx/redlineParser.ts` 追加（import 区合并加 `loadDocxZip` 与 xmlAst 工具）：

```typescript
import { loadDocxZip } from './zipRewriter'
import { parseOoxml, findFirst, findAll, getAttr, textOf } from './xmlAst'
```
...
export async function parseRedlineMarks(docxBuffer: Buffer): Promise<ParsedRedlineMarks> {
    ...
    if (docFile) {
        ...
        for (const n of findAll(ast, 'w:t')) rawT += textOf(n)
        for (const n of findAll(ast, 'w:delText')) rawDel += textOf(n)
    }
    ...
    return {
        ...
        survivingInsIds,
        survivingDelIds,
        corpusT: normalizeForMatch(rawT),
        corpusDel: normalizeForMatch(rawDel),
    }
}
```
...
git commit -m "feat(contract): 新增修订标记回传解析器"
```
...
新建 `tests/server/assistant/contract/uploadClientVersion.redline.test.ts`。构造一条 `completed` 状态的 `contractReviews` + 若干带 `problematicQuote`/`suggestedClauseText` 的 `contractRisks` + 对应 `contractAnnotations`（带 `wordCommentRef`）；用 `injectRedlineMarks` 产出修订版 docx；分别模拟「未处理」（原样回传）与「全接受」（把 docx 里 `<w:ins>` 解包、`<w:del>` 整体删除后回传）上传，断言：

```typescript
it('修订版回传：未处理 docx 不触发防误删、风险标记 untouched', async () => {
    // ...上传未经 Word 处理的修订版 docx，消费 SSE 到 complete
...
> 完整用例集见 spec §11.2；本步骤先落上述 2 个核心用例驱动实现，其余在 Step 6 补全。
...
import 区加：
```typescript
import { parseRedlineMarks, classifyRedlineDecision, type ParsedRedlineMarks } from './docx/redlineParser'
import { ClientRedlineDecision } from '#shared/types/contract'
```
...
定位 Step 3 里现有的 `systemDbAnnotations` / `matchRatio` / `NO_MATCH_THRESHOLD` / `tripsSafety` 计算块（`const systemDbAnnotations = dbAnnotations.filter(...)` 到 `tripsSafety` 赋值结束），整段替换为：
...
同步把该 `logger.error` 里日志字段从 `matchRatio` 等改为 `coverageRatio` / `identifiableRiskCount: identifiableRiskIds.size` / `coveredCount`（保持日志可读）。

- [ ] **Step 5: 修订处置识别块**

在 `tripsSafety` 的 `return` 之后、Step 4（AI 增量审查）开始之前，加修订识别块：
    ...
    if (redlineUsable) {
        ...
        for (const ref of redline!.refs) {
            ...
            const decision = classifyRedlineDecision({
                ...
                corpusT: redline!.corpusT,
                corpusDel: redline!.corpusDel,
...
- Test: `tests/app/components/assistant/contract/RiskListPanel.test.ts`（追加用例）

- [ ] **Step 1: 写失败测试**

在 `tests/app/components/assistant/contract/RiskListPanel.test.ts` 追加用例（沿用该文件挂载 RiskListPanel、传 `risks` prop 的方式）：

```typescript
it('风险带 clientRedlineDecision 时渲染客户处置徽章', async () => {
    const wrapper = mountRiskListPanel({
        risks: [makeRisk({ id: 1, clientRedlineDecision: 'accepted' })],
    })
    expect(wrapper.text()).toContain('客户已采纳')
})
```

Run: `npx vitest run tests/app/components/assistant/contract/RiskListPanel.test.ts --reporter=verbose`
...
(a) import 区 —— `#shared/types/contract` 的 type import 加 `ClientRedlineDecision`，并加值 import：
```typescript
import { RISK_LEVEL_LABEL, ClientRedlineDecision, ClientRedlineDecisionText } from '#shared/types/contract'
```
lucide 图标 import 加 `XCircleIcon, CircleDashedIcon, HelpCircleIcon`（`CheckCircle2Icon` 已有）。
...
```bash
git add app/components/assistant/contract/ContractReviewPanel.vue app/components/assistant/contract/RiskListPanel.vue tests/app/components/assistant/contract/RiskListPanel.test.ts
...
- 修订身份证 `word/customXml/redlineRefs.xml`（结构、`injectRedlineMarks` 写入、`customXmlRegistrar` 注册）。
- 回传识别：`redlineParser.parseRedlineMarks` + `classifyRedlineDecision` 双层算法（w:id 精确层 + 正文比对兜底层）、4 态判定（accepted/rejected/untouched/ambiguous）。
- `uploadClientVersion` 的修订识别接入点（Step 2/3b/5）、统一覆盖率安全保护、`NO_CONTENT_MATCH` 报错。
- 自定义署名：`users.contractExportSignature` + `resolveContractExportSignatureService`，导出端 AI 修订/批注作者名用署名、去 `LS:` 前缀。
...
非自动化步骤，记录于此供执行者手动完成：导出一份修订版 docx → 在真实 Word 里分别做「接受全部 / 拒绝全部 / 接受部分」并保存 → 各回传一次，确认 `clientRedlineDecision` 落库正确、横幅统计准确、`redlineRefs.xml` 经 Word 保存后保留。若 Word 重排了 `w:id`，确认正文比对兜底层仍判定正确。
