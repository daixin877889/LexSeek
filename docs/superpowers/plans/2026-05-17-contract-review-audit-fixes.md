...
## 复核结论（2026-05-17 二次核对）

> 本节是计划成稿后、由 6 个并行核对员对照当时 `dev` 分支代码逐条复核的产出。**结论：计划高度可信，可直接作为修复依据。** 复核只读代码、不改代码。

### 总体

44 个待修问题全部复核：**40 个完全成立、4 个部分成立（问题真实存在，仅描述/根因有小偏差，见下）、0 个误报、0 个已被悄悄修掉**。第三节 orphaned 的 4 步根因链逐步复核全部成立 —— 含「Step 5 锚点迁移完全弃用 `redlineRefs.xml` 的 `paraIdxs`」这一核心设计缺陷。三个「需核实项」V1（分析阶段不计费）、V2（总览要点点击联动失效）、V3（合同对话静默卡死）均确认为真实问题；M12（立场风险字段丢失）仍需产品决策（加列持久化 vs 放松 schema）。

第二节 6 条「已修复」声明：第 1 / 2 / 3 / 6 条修复彻底；第 4、5 条对主路径修复彻底、但各留一处边界缺口 —— 这两处缺口恰好就是第四节的 **M7、L6**，已透明登记、无隐瞒。

### 四处需在执行前校准的描述（已就地补注到对应问题项）

- **S1** —— 根因不止「`warnings` 被丢弃」。真正缺陷是没有任何代码区分「真无风险」与「条款全部分析失败」。修复范围比原文大：除补解构 `warnings`，还须在 `runAnalyzeLoop` 返回后加「全失败 → 置 failed」判断，且 `runAnnotateAndUpload` 的「无风险无批注 → completed」分支需配合改。
- **M14** —— 抛错触发点更正：不是「comment 计数 = 原生 + 新注入」，而是 `document.xml` 残留的客户原生 `commentRangeStart/End/reference` 让校验三项计数超标。结论（带原生批注合同无法 comment 导出）不变。
- **M17** —— 非独立问题，是 M14 在「下载历史回传版本」链路上的连带，comment 模式为确定性必然抛错。**应并入 M14 一起修，不单列。**
- **M8** —— 「三处口径不一致」更正为「一处与另两处不一致」：`collectNonEmptyParagraphs` 与 `buildCommentAnchorMap` 口径完全相同（均「body 直接子 `<w:p>`」、不递归表格），仅 `parseContractDocx`/`paragraphsFromAst` 递归含表格段落而不同。问题在含表格合同上才触发。

### 三个影响修复策略的全局发现

1. **回滚不对称是系统性问题** —— S4 / M2 / M3 / M6 / L8 同源：回传服务的补偿逻辑只能撤销新建行，对 Step 4a 原地覆盖的存量数据、事务已提交的改动、Step 4b 半成品都无能为力。**第三批建议统筹设计一套覆盖全链路的事务/补偿边界，而非逐条打补丁。**
2. **S6 / M18 / V3 同根** —— 业务错误统一裹在 HTTP 200 里，前端/SDK 用 `resp.ok` 判失败永远为真。其中 **V3 的失败点在第三方 SDK（`FetchStreamTransport`）内部、无法改 SDK 源码** —— 修 V3 需额外决策（让 SSE 接口「开流前失败」真返回非 200 状态码，或在包装层拦 `content-type`），S6 的修法不能直接套用。
3. **S5 与 M8 必须一起改** —— S5 推荐的 redline-aware 确定性迁移用 `ref.paraIdxs`（`collectNonEmptyParagraphs` 口径），而 `buildClauseToParagraphMap` 用 `parseContractDocx` 口径；含表格合同两者错位，确定性迁移会落错段。做 paraIdx → segment 逆映射时必须统一口径。

### 行号漂移

文档行号基于 2026-05-17 代码快照，已出现漂移（如 M13 `418-451`→`415-451`、L2 常量 `218`→`208`、M5 `253-326`→`279-326`）。文件名仍准确；执行时一律以实际代码为准重新定位。
...
comments.xml 整体覆盖。`writeTextToZip('word/comments.xml', buildCommentsXmlFromAnnotations(...))` 只含我方批注，原文档既有 `<w:comment>` 被丢，但 document.xml 里它们的 commentRangeStart/End/reference 不会被清。`assertCommentIntegrity`（约 513-540）统计 rangeStart 数 = 原生 + 新注入 ≠ validCount → 抛错 → **带客户原生批注的合同永远无法以 comment 模式导出**。**修复**：保留原生 `<w:comment>` 并合并，或清理原生 comment 的残留标记。 **【核对修正 2026-05-17】** 抛错触发点更正：`assertCommentIntegrity` 数 `w:comment` 的那项其实对得上；真正抛错是 `document.xml` 里残留的客户原生 `commentRangeStart/End/reference` 让这三项计数超标。结论不变。
...
base = 客户回传 docx 时重复注入。下载「客户回传」类版本时 base = 回传原件，已含我方上一轮注入的批注/修订标记，重新注入会叠加或触发 `assertCommentIntegrity` 抛错。**修复**：确认回传是否产出「干净 base」，否则历史版本下载对回传版本会失败/产出脏文件。 **【核对修正 2026-05-17】** 此问题非独立项，是 M14 在「下载历史回传版本」链路上的连带 —— comment 模式为确定性必然抛错（`inject_failed`）。应与 M14 合并处理，修 M14 时一并覆盖下载链路。
...
>
> **开工前先读「复核结论（2026-05-17 二次核对）」一节**：S1 根因已扩大（不止「丢 warnings」）、M17 应并入 M14、第三批 S4/M2/M3/M6/L8 的补偿逻辑建议统筹设计而非逐条打补丁。
