...
- `tests/server/assistant/document/documentTemplate.service.test.ts` — 更新路径断言
 ...
 * useRuntimeConfig()。独立 tsx 脚本不要用本函数。
 */
...
- [ ] **Step 1: 更新 documentTemplate 测试断言**

`tests/server/assistant/document/documentTemplate.service.test.ts` 中现有断言期望裸 `global-templates/` 前缀（约 304-353 行）。测试环境 `basePath=test/`，全局模板新路径为 `test/system/document_template/...`。

把该测试文件里所有 `global-templates/` 相关断言改为新前缀。两处典型断言改法：
...
若文件里还有针对 `scope='user'` 模板路径 `users/{id}/templates/` 的断言，一并改为 `test/user{id}/document_template/`。打开文件搜索 `global-templates`、`templates/` 逐处核对修正。

- [ ] **Step 2: 运行测试确认失败**

Run: `npx vitest run tests/server/assistant/document/documentTemplate.service.test.ts --reporter=verbose`
...
```bash
git add server/agents/document/documentTemplate.service.ts server/agents/document/documentExport.service.ts tests/server/assistant/document/documentTemplate.service.test.ts
...
加 import `import { buildStorageKey } from '~~/server/utils/storagePath'`；确认 `FileSource` 已 import。
...
```ts
    // 上传到 OSS：{env}/user<userId>/caseAnalysis/version-<versionId>-<uuid>.docx
    const ossPath = buildStorageKey({
        scope: 'user',
...
Run: `npx vitest run tests/server/contract tests/server/agents/contract --reporter=verbose`
Expected: PASS（若部分目录不存在，vitest 会忽略；关注无因路径改动导致的回归）。如有断言裸 `contract-review/` 前缀的测试，按 `test/user{id}/caseAnalysis/` 更新后再跑。
...
Run: `npx vitest run tests/server/services/agent-platform/tools --reporter=verbose`
Expected: PASS（关注 `uploadWorkspaceFile` 相关用例无回归；若有断言裸 `users/`/`temp/` 前缀的，按新路径更新）。
...
Expected: PASS。`tests/server/services/material/image-processor.test.ts`、`tests/server/material/mineruResult.service.test.ts` 若有断言裸路径或依赖 `basePath` 的用例，按新前缀（测试环境 `test/`）更新后再跑。
 ...
 *
 * 用法：
 *   npx tsx server/scripts/migrateOssBasePath.ts                 # dry-run，仅打印待迁移清单
 *   npx tsx server/scripts/migrateOssBasePath.ts --execute       # 实际复制 + 改库（源对象保留）
 *   npx tsx server/scripts/migrateOssBasePath.ts --delete-source # 删除已迁移成功的根目录旧对象
 *
...
async function main() {
    // ---- 脚本引导：独立 tsx 运行不在 Nitro 运行时，先挂全局依赖再动态 import 服务层 ----
    ...
    for (;;) {
        ...
        for (const row of rows) {
            ...
            if (deleteSource) {
                ...
                try {
                    const oldMeta = await client.getObjectMeta(oldKey).catch(() => null)
                    if (!oldMeta) { skipped++; continue }
                    const newMeta = await client.getObjectMeta(filePath).catch(() => null)
                    if (!newMeta) {
            ...
            try {
                const meta = await client.getObjectMeta(oldKey).catch(() => null)
                if (!meta) {
                    sharedLogger.warn(`源对象不存在，跳过 id=${row.id} ${oldKey}`)
                    skipped++
                    continue
                }
                const size = Number(meta.res.headers['content-length'] ?? 0)
...
Run: `npx tsx server/scripts/migrateOssBasePath.ts`
...
```bash
# 每个环境用其自己的 .env
npx tsx server/scripts/migrateOssBasePath.ts                 # 1. dry-run 查清单
npx tsx server/scripts/migrateOssBasePath.ts --execute       # 2. 复制 + 改库
# 3. 人工抽查若干文件可正常下载
npx tsx server/scripts/migrateOssBasePath.ts --delete-source # 4. 确认无误后删根目录旧对象
```
