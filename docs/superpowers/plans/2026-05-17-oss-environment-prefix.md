...
**更新断言（documentTemplate.service.test.ts）**：现有断言期望裸 `global-templates/` 前缀（约 304-353 行）。桩已把 `basePath` 设为 `test/`，全局模板新路径为 `test/system/document_template/...`。逐处改：
...
若文件里还有针对 `scope='user'` 模板路径 `users/{id}/templates/` 的断言，一并改为 `test/user{id}/document_template/`。搜索 `global-templates`、`templates/` 逐处核对。

**更新断言（documentExport.test.ts）**：第 278 行附近有 `expect(...).toContain('users/100/document-exports/')`，改为：

```ts
expect(...).toContain('test/user100/document_export/')
```

> 该文件第 135/147/154 行的 `MOCK_UPLOAD_RESULT.name` / `MOCK_OSS_FILE.filePath` 是 mock 返回值（被测代码原样透传给 `createOssFileDao`），不影响断言，无需改动。
 ...
 */
async function getObjectSize(client: any, key: string): Promise<number | null> {
    try {
