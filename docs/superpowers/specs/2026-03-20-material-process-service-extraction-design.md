...
4. OSS 文件检查：
   - 无 `ossFileId` 但有 `content` → 直接返回 `{ id, status: COMPLETED }`
   - 无 `ossFileId` 且无 `content` → 抛 `MaterialProcessError(400)`
5. 获取 OSS 文件信息，不存在抛 `MaterialProcessError(404)`
6. 更新状态为 `PROCESSING`
7. 按 `material.type` 分发到私有函数：
...
8. 处理失败 → 更新状态为 `FAILED`，抛 `MaterialProcessError(500)`
9. 有内容 → 调用 `updateMaterialContentService` 更新内容
10. 向量化（`enableEmbedding !== false` 时）→ 失败仅 logger.error，不阻塞主流程
11. 返回 `ProcessMaterialResult`
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const message = result.contentLength ? '材料处理成功' : '材料处理已提交，请稍后查询结果'
    return resSuccess(event, message, result)
  } catch (error: any) {
    if (error instanceof MaterialProcessError) {
      return resError(event, error.statusCode, error.message)
    }
