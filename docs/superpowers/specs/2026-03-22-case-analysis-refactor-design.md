...
| 工具 | 调用时机 | 职责 |
|------|---------|------|
| `reserve_points` | 用户选择模块后 | 预扣积分，返回 reservationId |
| `confirm_points` | 单个子代理成功后 | 确认该模块积分实扣 |
| `rollback_points` | 子代理失败时 | 回滚该模块积分预扣 |
...
3. 计算总 token 量
4. 决定提供模式：
   - `totalTokens < threshold` → 全量提供（返回完整文本）
   - `totalTokens >= threshold` → 摘要模式（返回材料列表+摘要）
...
│              │          │            │        │  rollback_points]         │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ summary      │ subagent │ deepseek-1 │ 1      │ [search_case_materials,   │
│              │          │            │        │  search_law]              │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ chronicle    │ subagent │ deepseek-1 │ 2      │ [search_case_materials]   │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ claim        │ subagent │ deepseek-1 │ 3      │ [search_case_materials,   │
│              │          │            │        │  search_law]              │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ trend        │ subagent │ deepseek-1 │ 4      │ [search_case_materials,   │
│              │          │            │        │  search_law]              │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ cause        │ subagent │ deepseek-1 │ 5      │ [search_law]              │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ defense      │ subagent │ deepseek-1 │ 6      │ [search_case_materials,   │
│              │          │            │        │  search_law]              │
├──────────────┼──────────┼────────────┼────────┼───────────────────────────┤
│ evidence     │ subagent │ deepseek-1 │ 7      │ [search_case_materials]   │
└──────────────┴──────────┴────────────┴────────┴───────────────────────────┘
```
...
| 表 | 变更 | 说明 |
|---|------|------|
| nodes | 新增 `type='subagent'` | 区分主代理和子代理 |
| nodes | priority 字段 | 子代理执行排序 |
| caseMaterials | 新增 `summary` 字段 | 材料摘要缓存 |
| pointReservations | 新建表 | 积分预扣记录 |
| LangGraph Store 表 | 自动创建 | PostgresStore 长期记忆 |
...
model PointReservation {
  ...
  userId        Int
  sessionId     String
  moduleName    String    // cause, defense, ...
  amount        Int       // 预扣积分数量
  status        Int       @default(1) // 1=预扣 2=已确认 3=已回滚
...
cases, caseSessions, caseAnalyses, caseMaterialEmbeddings, nodes, prompts, nodeGroups, levelNodeAccess, checkpoints 系列表
...
| NodeConfig/NodeService | 复用并扩展 | 新增 `getSubagentConfigs()` 方法，按 type='subagent' + priority 查询 |
