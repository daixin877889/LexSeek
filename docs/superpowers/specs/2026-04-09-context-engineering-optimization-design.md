...
对 LexSeek 项目中工作流和 Agent 的上下文工程进行全面审查后，发现 13 个问题横跨工具层、提示词层、中间件层、上下文层和基础设施层。本设计采用"逐层加固"策略，从底层向上修复，每层的改动独立可测试。
...
function truncateToTokenLimit(content: string, maxTokens: number): string {
  ...
  while (lo < hi) {
    ...
    if (countTokensSync(content.slice(0, mid)) <= maxTokens) {
      lo = mid
    } else {
      hi = mid - 1
    }
...
const triggerTokens = Math.floor(contextWindow * 0.6)
...
```typescript
import { compressMessages, safetyTrimMessages } from '../context/messageCompressor'
...
export function safetyTrimMiddleware(options: { model: BaseChatModel; maxTokens: number }) {
  return {
    ...
    beforeAgent: async (state: { messages: BaseMessage[] }) => {
      ...
      // 防线一：LLM 摘要压缩
      try {
        const compressed = await compressMessages(state.messages, {
          model: options.model,
          contextBudget: options.maxTokens,
        })
...
**接口签名不变**，调用方无需修改。移除原有的 `Map<string, Record<string, any>>`。
