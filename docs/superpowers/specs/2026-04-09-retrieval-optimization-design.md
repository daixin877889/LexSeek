...
-- tsvector 计算列（自动跟随 text 列更新）
ALTER TABLE law_embeddings
  ADD COLUMN IF NOT EXISTS tsv tsvector
  GENERATED ALWAYS AS (to_tsvector('chinese', COALESCE(text, ''))) STORED;
...
async function fullTextSearch(
    ...
    const query = `
        SELECT text, metadata,
               ts_rank_cd(tsv, to_tsquery('chinese', $1)) as score
        FROM ${tableName}
...
混合通道的简化版：**只走 Vector 检索 + Rerank**，不走 BM25。适用于纯自然语言查询。
...
- `models` 表新增 Rerank 模型记录，type 标记为 `rerank`
...
async function retrievalRouterService(
    ...
    switch (intent.intent) {
        case 'exact':
            ...
            if (results.length === 0) {
                results = await hybridSearch(intent, request)  // 降级
            }
            break
        case 'hybrid':
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    && apt-get purge -y gcc make git postgresql-server-dev-17 \
...
| `server/services/legal/searchLaw.tool.ts` | 内部逻辑改为调用 retrievalRouter |
| `server/services/material/materialPipeline.service.ts` | `getMaterialContextService` 改为分级注入 |
