...
- **中断处理**：四个消费方各自实现 `__interrupt__` 解包 + `Dialog + InsufficientPointsCard` 包装
...
- **Redis 已有依赖**：项目通过 `server/lib/redis.ts` 提供 Redis 客户端，`agentEventBridge.ts`、`agentRun.service.ts`、`agentWorker.ts` 等均使用 Redis。本次新增的并发防重可直接复用

### 1.5 范围说明

`app/pages/dashboard/analysis/[sessionId].vue`（案件分析页）使用独立的 `reactive(useStream(...))` 创建流，并有自己的 `interrupt` / `interruptData` 解包逻辑（第 157-169 行）和 Dialog 包装（第 15-31 行）。该页面纳入 Phase 4 的对齐范围，改为使用 `useStreamChat` 底层和 `<InterruptHandler>` 组件。
...
**影响**：`useInitAnalysis` 改造后需要将所有直接访问 `stream.xxx` 的地方改为 `stream.xxx.value`。这是一个明确的、可机械替换的改动，不涉及逻辑变化。

选择此策略的原因：`useCaseChat`（已被小索 + 模块对话 + 案件分析页三个消费方使用）的 ComputedRef 模式更通用，且 Vue 3 Composition API 的主流约定就是通过 `.value` 访问响应式数据。

#### `interruptData` 统一解包

完整伪代码（合并四个消费方的解包逻辑为一步）：

```typescript
// 输入：useStream 的 stream.interrupt（可能是数组或单对象）
// 输出：解包后的 interrupt 数据（如 { type: 'insufficient_points', data: {...} }）
interruptData: computed(() => {
  const raw = stream.interrupt  // useStream 原生 getter
  if (!raw) return null
  
  // 步骤1：从数组中取首项（与现有 CaseDetailXiaosuo:53-57 和 useInitAnalysis:73-76 一致）
  const first = Array.isArray(raw) ? raw[0] : raw
  
  // 步骤2：取 .value 属性（LangGraph interrupt 的标准包装层）
...
四个消费方的 Dialog + InsufficientPointsCard 包装代码替换为单行：
```vue
...
**`AnalysisModuleChat.vue`**（133 行 → ~80 行）：使用 `ChatWindowShell`（传入 `positionOffset` 使模块对话窗口偏移避免与小索重叠）+ `SessionListPopover`（新增多 session 支持）+ `InterruptHandler`。

**`AnalysisModuleChatBar.vue`**：多 session 模式下，状态条逻辑需调整——每个模块的 manager 内部可能有多个 session，状态条仅需显示"该模块当前是否有活跃分析"（由 manager 的 `isLoading` computed 提供），不需要展示每个 session。改动量小（~10 行）。
...
| PR #2 | 单元测试 | `useStreamChat`（computed 包装、interruptData 解包完整链路）、`useChatSessionManager`（session CRUD、switchSession、stopGeneration 双重取消、竞态防护、init 幂等保护）、`useStopActiveRun` |
...
| PR #4 | 回归测试 | `useInitAnalysis` 的 watch(values) 触发时机、moduleStates 推断、mergedResult 合并逻辑——**先写对比测试确认改造前后行为一致**。`analysis/[sessionId].vue` 的 interrupt 处理验证 |
