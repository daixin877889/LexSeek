...
// P2002 冲突只可能来自"两个并发请求都走到 create" —— 不在此处 catch，
// 让整个事务回滚并传播为 500，由前端 toast 提示用户手动重试（见下方"并发冲突处理"）。
...
| findFirst 结果 | `existing.deletedAt` | 动作 | 识别克隆 |
|---|---|---|---|
| null（无记录） | — | 创建新 ossFile（走第 3 步） | 需要（第 4 步，仅识别记录；嵌入在分析启动时由 ensureMaterialsReady 延迟生成） |
| 非 null，未软删 | null | 直接复用已有 ossFileId | 无需（原识别记录仍在） |
| 非 null，已软删 | 非 null | 复活：UPDATE `deletedAt=null` 翻转回正常 | 无需（识别记录未级联清理，仍在原地） |
...
async function applyDemoCase(example: ExampleItem) {
  ...
  try {
    const data = await useApiFetch<{
      content: string | null      files: OssFileItem[]
...
**实施阶段重构提示**：`applyDemoCase` 和 `confirmReplaceExample` 的"prepare 请求 + setText + addFiles"核心逻辑高度重复，实施时应抽为一个私有函数 `fetchAndFillFromDemoCase(example)`；`applyDemoCase` 直接调用该函数，`confirmReplaceExample` 在拿到 data 后调用 reset + 该函数的填充部分。这个抽取与 spec 4.2 已规划的"交互逻辑下沉到 `useCaseCreation` composable"一并完成。
...
| 用户并发两次点同一 demo case | 前端 `preparingDemoCaseId` 非空时忽略后续点击；若前端防御失效（两个页面 tab），后端两个 prepare 请求其中一个会因联合唯一约束 `idx_oss_files_user_bucket_path` 触发 P2002 冲突，事务回滚传播为 500 错误，前端 toast 提示，用户手动重试一次即成功（第二次 findFirst 会命中对方已 commit 的行） |
...
- [ ] 使用 demo case 创建的案件启动分析时，`ensureMaterialsReadyService` 自动为克隆的材料生成嵌入向量（验证：分析启动后查 `case_material_embeddings` 有对应 userId/sourceId 的记录），分析流程正常完成
