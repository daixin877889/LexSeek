...
**生命周期**：首次调用 `write_skill_file` 时自动创建。workspace 绑定 session 生命周期（支持多轮迭代修改），24h 无新活动时由定时任务兜底清理。
...
| 场景 | 状态 | Agent 行为 |
|------|------|-----------|
| 用户在同一轮对话中要求修改 | workspace 文件存在 | 读取 workspace 中的脚本，修改后重新执行 |
| 用户在后续对话中要求修改（24h 内） | workspace 文件存在 | 同上，workspace 会话级持久 |
| 用户在 24h 后要求修改 | workspace 已清理 | 根据 checkpoint 对话历史重新生成脚本并执行 |
| 用户从云盘删除文件后要求重新生成 | 云盘文件不存在 | 重新生成并重新上传到用户云盘 |
| 用户点击历史消息中已删除文件的下载按钮 | 云盘文件不存在 | 前端文件卡片显示"文件已被删除" |

**关键点**：workspace 和用户云盘是独立存储。用户删除云盘中的文件不影响 workspace（反之亦然）。Agent 通过 checkpoint 对话历史可重建上下文，即使所有文件都丢失也能重新生成。
...
**"保存到云盘"按钮**：用户清理云盘腾出空间后，点击此按钮将临时文件转存到个人云盘。

```
POST /api/v1/files/save-temp
Body: { tempFileId: "temp-12345" }
```

后端逻辑：
1. 检查临时文件是否仍存在（未过期）
2. 检查用户云盘配额是否足够
3. 从临时区域复制到用户 OSS 目录 + 创建 files 表记录 + 扣减配额
4. 删除临时文件
5. 返回新的永久 fileId → 前端更新卡片状态为"已保存"

失败场景：
- 临时文件已过期 → 提示"文件已过期，请重新让 AI 生成"
- 配额仍不足 → 提示"云盘空间仍不足，请继续清理后再试"
