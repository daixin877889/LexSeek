...
└────────────────┬────────────────────────────────────────────┘
   ...
   └─ 否则 → 异步任务 memoryExtractionTask（fire-and-forget）
            ├─ invokeNodeJsonService('caseMemoryExtract', { messages })
...
```ts
// server/services/agent-platform/middleware/afterAgent.middleware.ts
afterAgentMiddleware = (ctx) => ({
  name: 'afterAgent',
  afterAgent: async (state) => {
    ...
    void runMemoryExtractionTask({ caseId, sessionId, messages: state.messages })
      .catch(e => logger.warn('memoryExtraction failed', { e }))
  },
})

async function runMemoryExtractionTask({ caseId, messages }) {
  try {
    const result = await invokeNodeJsonService('caseMemoryExtract', { messages, caseProfile })
    for (const m of result.memories) {
      // 软去重（新增 dao 函数 findActiveMemoryBySubjectDAO）：
      //   raw SQL 查 metadata->>'caseId' + metadata->>'subjectKey' + invalidatedAt IS NULL
      // similarText：简单实现（完全相等或 Levenshtein 距离 < 阈值）
      if (m.subject_key) {
        const existing = await findActiveMemoryBySubjectDAO(caseId, m.subject_key)
        if (existing && similarText(existing.text, m.text)) continue
      }
    ...
    if (e.code === 'NODE_NOT_FOUND') {
      logger.warn('caseMemoryExtract 节点未配置，skip afterAgent')
      return
    }
    throw e
  }
...
**新增 DAO 函数**（`server/services/memory/memory.dao.ts`）：
- `findActiveMemoryBySubjectDAO(caseId, subjectKey)` — raw SQL 查 metadata
- `listMemoriesDAO(caseId, filter, cursor, limit)` — 列表查询，按 metadata->>'createdAt' DESC
- `softDeleteMemoryDAO(memoryId)` — jsonb_set 设 metadata.invalidatedAt

**新增工具函数** `similarText(a, b): boolean`（简单 Levenshtein 距离比 / 阈值 0.9 或精确匹配 trim 后相等）。

#### LangChain 框架适配

LangChain 的 `AgentMiddleware` 不原生支持 `afterAgent` hook。在 `server/services/agent-platform/factory/runtime.ts` 包一层：

```ts
const result = await agent.invoke(input, config)

// ↓ 新增：执行 afterAgent hooks
for (const mw of middlewareStack.filter(m => m.afterAgent)) {
  try { await mw.afterAgent({ ...ctx, state: result }) }
  catch (e) { logger.warn(`afterAgent[${mw.name}] failed`, { e }) }
}

return result
```

afterAgent 失败 catch 静默吞掉，不影响主响应。
...
#### `GET /api/v1/case/memories/:caseId`
...
#### `POST /api/v1/case/memories/:caseId`

**Body**：`{ text, kind, subjectKey? }`  
**逻辑**：subjectKey 空时调 `caseMemorySubjectInfer` 节点推断；写入 source='manual_user'。
...
5 个核心 tab 不动，最右加 ⋯ 按钮（`MoreHorizontalIcon`），点击弹出 shadcn `<Sheet side="bottom">`：
...
mock `invokeNodeJsonService`（不调真实 LLM），但走真实 DB：
...
```
T1 类型扩展 + 节点种子数据（MemoryWriteInput.source 类型扩展 / CaseMemoryMetadata.source 扩展 / nodes + prompts seedData.sql 加 2 行）
   ↓
T2 后端基础（writeMemoryService 兼容 source / similarText / 新增 caseMemory.dao.ts：findActiveMemoryBySubjectDAO + listMemoriesDAO + softDeleteMemoryDAO）
   ↓
T3 后端 API（GET/POST/DELETE 3 个端点 + 权限校验）
   ↓
T4 afterAgent 中间件（agent-platform/middleware + runtime.ts 包一层）
   ↓
T5 invokeNodeJsonService 集成（caseMemoryExtract / caseMemorySubjectInfer）
   ↓
...
| afterAgent 框架适配脆弱 | 中 | LangChain 不原生支持，工厂层包一层 | 实现 + 单测覆盖；LangChain 升级时主动检查适配点 |
...
- `invokeNodeJsonService`：`server/services/agent-platform/tools/invokeNodeJson.ts`
