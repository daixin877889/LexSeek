...
新增响应式状态（与同文件既有 `subThreadsMap` / `syntheticToolCalls` 风格一致，用 `reactive<Record>` 而非 `ref<Map>`）：
...
行为：
- `resolveInterrupt(value)` 调用前：把 `{ interrupt: interruptData.value, resumeValue: value, resolvedAt: new Date() }` 写入 `resolvedInterrupts[interruptData.value.toolCallId]`
- `switchSession()` / `init()` 切换会话时：清空字段（`for (const k in resolvedInterrupts) delete resolvedInterrupts[k]`）
...
调用方（`CaseDetailXiaosuo` / `AssistantChatPanel` / `ContractReviewPanel`）从 `useStreamChat` 解构后：

```ts
provide('messageStreamContext', {
  interruptData,         // Ref
  resolvedInterrupts,    // reactive Record
})
```
...
| 测试文件 | 现状 | 改动 | 用例 |
|---------|------|------|------|
| `tests/client/composables/useStreamChat.test.ts` | 已存在 | 扩展 | resolve 后 `resolvedInterrupts[toolCallId]` 正确填充；session 切换后清空 |
...
| 文件 | 改动类型 |
|------|---------|
| `app/composables/useStreamChat.ts` | 修改：加 `resolvedInterrupts` reactive Record + 在 resolveInterrupt 写入 + session 切换清空 |
...
| `tests/client/composables/useStreamChat.test.ts` | 扩展：resolvedInterrupts 用例 |
