...
- 用户在小索 / 法律助手对话时调起 `draft_document` 工具
...
实测线上 dev 库:`document_drafts.status='failed'` 21 条 / `'ready'` 51 条,**失败率约 27%**。一个具体复现案例:用户从法律助手讲完房屋租赁纠纷案情让起草起诉状,9.4 秒就 `status=failed`,前端报"draft_document: 文书 Agent 起草失败"。

定位到根因不止一个:

1. **工具同步嵌套子 Agent**违背"工具回归工具"原则,任何子流失败都被工具识别为整体失败
2. **法律助手对话上下文几乎全丢**——只通过 LLM 自动摘要的一句 `additionalContext` 传给 documentMain;documentMain 在新 thread 里看不到原始对话,什么都问不出来就失败
...
- 三个入口(小索 / 法律助手 / 文书工作区)起草文书行为统一、信息无损
- 取消"工具嵌套子 Agent"反模式,工具回归无会话纯函数
- documentMain 升级为平级主 Agent,跟小索 / 法律助手同构
...
| assistantMain(法律助手) | ASSISTANT | **新挂** legal-document-writer + 5 个其他 | 通用法律咨询 + 起草/修改文书 |
...
| `update_document_draft` | draftId / fieldUpdates / suggestions / aiTitle / reason | draftId / changedFields / summary | ✗ |
...
3. `createSnapshotService(draftId, 'ai-extract', { values, aiTitle })`
4. 处理 sourceFileIds(若有 → `ensureMaterialsReadyForDraftService`)
5. 应用 aiTitle(若有 + titleOverridden=false → `applyAITitleIfAllowedService`)
6. publish `DRAFT_SAVED` SSE custom event
7. 返回 JSON
...
**内部流程**:
1. 查 draft + 模板,得到 placeholders 名集合
2. 过滤 fieldUpdates,只保留 placeholders 范围内的 key
3. `merged = { ...existingValues, ...filteredUpdates }`
4. `updateDocumentDraftDAO(draftId, { values: merged, metadata: { suggestions } })`
5. publish `DRAFT_UPDATED` SSE custom event
6. 返回 changedFields 列表
...
documentMain_system 改成(运行时变量由 nodeConfig + 注入逻辑提供):
...
验证:`typecheck` + 全套测试 + e2e + 三入口手动验证(小索 / 法律助手 / 文书入口)
...
- **法律助手路径**(当前 bug 场景):法律助手对话 → 讲案情 → "起草起诉状" → 选模板 → 草稿落库,字段填得对(尤其"原告"/"被告"/"事实和理由"/"诉讼请求"用上了对话信息)
...
- `server/agents/document/templateRecommend.service.ts` — 给 recommend_template 用
- `server/agents/document/documentDraft.service.ts:createDraftService` — 给 save_document_draft 用
- `server/agents/document/documentDraft.dao.ts:updateDocumentDraftDAO` — 给 update_document_draft 用
- `server/services/material/materialPipeline.service.ts:ensureMaterialsReadyForDraftService` — 给 save_document_draft 处理 fileIds 用

### 12.3 沿用前端组件

- `app/components/agents/document/interrupts/TemplateSelectCard.vue` — recommend_template 的 interrupt UI
- `app/composables/agents/useDocumentAgent.ts` — 文书工作区 Agent 工厂
- `app/composables/document/useDocumentDraftFields.ts` — 字段表单(零改动)
- `app/pages/dashboard/document/drafts/[id].vue` — 文书工作区主页面(零改动)
