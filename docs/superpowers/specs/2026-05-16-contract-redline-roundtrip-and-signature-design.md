...
- **customXml 是 Word 不篡改的稳定区**：现有批注身份证机制（`annotationRefs.xml`）已在生产运行，项目代码自身断言 customXml 是「Word/WPS 不篡改的权威源」。给修订标记加同样的 customXml 身份证，这条路已被验证可行。
- **Word 接受/拒绝修订的 XML 变化由 OOXML 规范明确定义**：接受 `<w:ins>` = 保留文字、去标签；拒绝 `<w:ins>` = 文字连标签一起删；接受 `<w:del>` = 文字连标签一起删；拒绝 `<w:del>` = 文字恢复（`<w:delText>`→`<w:t>`）、去标签。识别算法据此设计，并配「正文比对」兜底层，使其不依赖 Word 的 `w:id` 是否稳定。
...
- 「客户修订处置」= 客户对每条 AI 改写的态度：采纳了 / 拒绝了 / 还没看。
...
- `shared/types/user.ts` — `UserInfo` 加 `contractExportSignature?: string | null`。
- `shared/types/contract.ts` —
  - 新增 `ClientRedlineDecision` 字符串枚举（`accepted` / `rejected` / `untouched` / `ambiguous`）+ 中文标签映射。
...
- **API**：复用 `PUT /api/v1/users/profile`。zod schema 加 `contractExportSignature: z.string().max(50).optional()`；`updateUserProfileDao` 加该字段写入；`formatUserResponseService` / `GET /api/v1/users/me` 返回该字段。
...
- 新增 helper `resolveContractExportSignature(userId): Promise<string>` —— 读 `users` 的 `name` + `contractExportSignature`，按 4.1 规则算出署名。
...
**`injectAnnotations`**：`InjectAnnotationsOptions` 加 `signature: string`。`buildCommentsXmlFromAnnotations` 按 annotation 的 `authorType` 决定 `w:author`：

| authorType | `w:author` | 头像缩写 `w:initials` |
|---|---|---|
| `ai` | `signature` | 取 `signature` 前 2 字符 |
| `lawyer` | `a.authorName`（律师姓名） | `律` |
| `external` | `a.authorName`（客户名） | `客` |

**去 `LS:` 前缀**：现有 `buildAuthorField` 一律给作者名加 `LS:` 前缀（`LS` 即 LexSeek，也是泄露点）。本次重构为按上表产出干净作者名，不再加 `LS:` 前缀。

> 去前缀不影响回传识别：回传匹配以 customXml 身份证为主防线，不依赖 `w:author`。`w:author` 内的身份证后缀在 spec §14（前序设计）已移除，作者名当前纯展示用途。
...
<lexseekRedlineRefs xmlns="urn:lexseek:contract-review-redline:v1">
  <ref riskId="42" reviewId="871" delIds="5" insId="6"/>
  <ref riskId="43" reviewId="871" delIds="8,10" insId="11"/>
</lexseekRedlineRefs>
```

| 属性 | 含义 |
|---|---|
| `riskId` | `contractRisks.id` |
| `reviewId` | 归属审查 id，回传时用于拒绝跨审查 docx 串扰 |
| `delIds` | 该风险所有 `<w:del>` 节点的 `w:id`（跨段风险有多个，逗号分隔） |
| `insId` | 该风险 `<w:ins>` 节点的 `w:id`（跨段风险只有结尾段有 ins，单值） |
...
`injectRedlineMarks` 内部本就在装配时产出 `spansByRiskId: Map<riskId, RedlineWrapTarget>`（`riskId → paragraphSpans[{paraIdx, delId, insId}]`）。导出时用它直接构造并写入 `redlineRefs.xml`：

- 每条 risk 一个 `<ref>`：`delIds` = 所有 span 的 `delId`，`insId` = 唯一非 null 的 `insId`。
- `reviewId` 取 `options.reviewId`（已有入参）。
...
现有 `commentInjector.ts` 的 `ensureContentTypesRegistered` / `ensureDocumentRelsRegistered` 把 comments + annotationRefs 的注册写死在一起。本次抽出一个通用 customXml part 注册器（`server/agents/contract/docx/customXmlRegistrar.ts`），按 PartName 幂等注册/注销单个 customXml part，`redlineRefs` 与 `annotationRefs` 共用；`comments.xml`（非 customXml part）的注册保持现状。
...
1. 读 `word/customXml/redlineRefs.xml` → `RedlineRefEntry[]`（`{ riskId, reviewId, delIds: number[], insId: number }`）。文件不存在 → 空数组。
...
判定逻辑做成**纯函数** `classifyRedlineDecision(...)` 便于单测。

### 6.2 双层判定规则

对每条 `RedlineRefEntry`，先做归属校验，再判处置。

**归属校验**：`reviewId !== 当前 review.id` → 标记为跨审查串扰，不参与处置（见 §9）。

**Layer 1 · `w:id` 精确层**（最快路径）：

- `delIds` 全部在存活 del 集合、且 `insId` 在存活 ins 集合 → **未处理**。
- `delIds` 全部不在、且 `insId` 不在 → 进入 Layer 2 定 接受/拒绝。
- 其余（部分存活）→ 进入 Layer 2，结论不一致时落 `ambiguous`。

**Layer 2 · 正文比对层**（不依赖 `w:id`，对 Word 是否重排 id 免疫）：

取该 risk 的 DB 字段 `problematicQuote`（原文片段）与 `suggestedClauseText`（AI 改写），用 `textSimilarity.ts` 的 `normalizeForMatch` 归一化后做匹配——以归一化包含匹配为主，为容忍「客户接受后又轻微手改」辅以 `calcSimilarity` 相似度兜底。三个信号：

| 信号 | 含义 |
|---|---|
| `oldInDel` | `problematicQuote` 命中 `corpusDel` |
| `newInT` | `suggestedClauseText` 命中 `corpusT` |
| `oldInT` | `problematicQuote` 命中 `corpusT` |

判定表：

| `oldInDel` | `newInT` | `oldInT` | 判定 | 说明 |
|---|---|---|---|---|
| 是 | — | — | 未处理 | 删除标记还在，修订未走完 |
| 否 | 是 | 否 | **接受** | 只剩 AI 改写，原文已删 |
| 否 | 否 | 是 | **拒绝** | 只剩原文，AI 改写已撤 |
| 否 | 其它组合 | | **需确认** | 找不到/矛盾（如客户接受后又手改） |

> 修订模式下能进入 `redlineRefs.xml` 的风险，必定带非空 `problematicQuote` 与 `suggestedClauseText`（无锚点/无改写的风险在导出阶段就 fallback 成批注，不会出现在 `redlineRefs.xml`）。因此 Layer 2 不存在「比对字段为空」的情况。

**两层交叉**：Layer 1 给出「未处理」直接采纳（精确、免文本误差）；Layer 1 判「已处理」则以 Layer 2 结论为准；两层矛盾 → `ambiguous`。
...
- **Step 3 识别差异**：新增修订识别块——对每条 `RedlineRefEntry` 跑 §6.2 算法，产出 `Map<riskId, ClientRedlineDecision>` + 跨审查串扰列表 + 各类计数。安全保护改造也在此步（见 §9）。
...
- `systemRiskIds` = 带系统批注（`wordCommentRef != null`）的风险 id 集合。
- `coveredRiskIds` = （批注匹配上的风险 id）∪（`redlineRefs.xml` 中 `reviewId` 与本审查一致的风险 id）。
- `coverageRatio = |coveredRiskIds ∩ systemRiskIds| / |systemRiskIds|`。
- 中止条件：`systemRiskIds` 非空 **且** docx 含批注或修订 **且** `coverageRatio < 0.2`。

即：一条系统风险只要「批注匹配上」**或**「在修订身份证里登记过」，就算被本次 docx 覆盖，不计入误删风险。

### 9.2 跨审查串扰拦截

`redlineRefs.xml` 中存在 `reviewId` 与当前审查不符的条目 → 与现有 `crossReviewComments` 同等处理：该条目不参与处置，并在全部内容都跨审查/对不上时，报错指向「上传了别的合同的 docx」。
...
```
本轮变化：3 处外部变更 · 5 处条款修改 · AI 增量重审 5 条 · 客户修订：接受 12 / 拒绝 5 / 未处理 6
```
...
`RiskListPanel.vue` 每条风险加一个「客户处置」小徽章，复用现有风险徽章样式：

| `clientRedlineDecision` | 徽章文案 | 色调 |
|---|---|---|
...
数据流：回传完成后前端 `refreshWorkspace()` 重新拉 `GET /reviews/:id`，风险实体带上 `clientRedlineDecision` → 徽章渲染。需确保该接口的 risk 序列化包含新字段。徽章图标统一用 `lucide-vue-next`，不使用 emoji。
...
- `redlineParser`：解析 `redlineRefs.xml`（正常 / 缺文件 / 损坏 / 跨段 `delIds` 多值）；`classifyRedlineDecision` 纯函数覆盖判定表每一行（接受/拒绝/未处理/需确认）+ 两层矛盾 + 跨审查。
- `injectRedlineMarks`：导出后 `redlineRefs.xml` 内容正确（riskId/reviewId/delIds/insId）；`w:author` 用署名。
- `injectAnnotations`：AI 批注 `w:author` = 署名、`w:initials` 取署名前 2 字；律师/客户批注作者名不变；`LS:` 前缀已去除。
- `customXmlRegistrar`：part 注册/注销幂等；`both` 模式下 redline 与 annotationRefs 注册并存不互相清除。
...
导出一份修订版 → 在真实 Word 里分别做接受全部/拒绝全部/接受部分 → 回传，验证 `w:id` 存活情况与 `redlineRefs.xml` 的实际表现。重点确认 customXml 经 Word 接受/拒绝修订 + 保存后保留。若 Word 重排了 `w:id`，正文比对兜底层应仍能正确判定（设计已对此免疫）。
...
| 2 | 自定义署名：设置页 + `PUT /users/profile` + 导出端取值与应用 + 去 `LS:` 前缀 | 1 |
| 3 | 修订身份证导出：抽 `customXmlRegistrar` + `injectRedlineMarks` 写 `redlineRefs.xml` | 1 |
| 4 | 回传识别：`redlineParser` + `uploadClientVersion` Step 2/3/5 接入 + 处置落库 | 3 |
| 5 | 安全保护升级 + 报错文案修正 | 4 |
