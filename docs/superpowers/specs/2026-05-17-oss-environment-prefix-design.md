...
- 仅供 Nitro 运行时（API handler / Service / 中间件 / Agent 工具）调用 —— 独立运行的脚本里 `useRuntimeConfig()` 不存在，迁移脚本不使用本函数（见 3.4）。
...
**脚本引导与配置获取（独立运行约束）**：迁移脚本以 `bun run` 独立运行（bun 自动加载对应环境 `.env`，与现有脚本一致），**不在 Nitro 运行时内**，`useRuntimeConfig()` 不可用。因此：
