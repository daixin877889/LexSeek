...
| 4 | 合同审查 - 历史版本下载注入 | `server/agents/contract/contractReviewVersion.service.ts:387` | `contract-review/{userId}/version-{versionId}-{uuid}.docx` |
| 5 | 合同审查 - 审查结果持久化 | `server/agents/contract/middleware/reviewResultPersistence.middleware.ts:113` | `contract-review/{userId}/reviewed-{uuid}.docx` |
...
在 `server/services/storage/` 新增一个路径构造模块（建议文件 `storagePath.ts`），导出：
...
新增维护脚本 `server/scripts/migrateOssBasePath.ts`，按环境各跑一次（用对应环境的 `.env` 连各环境数据库、读各环境 `basePath`）：

迁移流程：

1. 查 `ossFiles` 中 `filePath` 非空、且**不以本环境 basePath 开头**的行；
2. 逐条对源 object 做 `headFile` 存在性检查（不存在则跳过并记日志）；
3. 在 OSS 内 `copy` 到 `${basePath}${filePath}`（复用 OSS client 的服务端 copy 能力）；
...
- `temp/` 临时文件（Agent 临时区、临时音频）不专门迁移历史数据 —— 靠 OSS 生命周期规则自然老化；但代码仍会修复，让新临时文件带环境前缀。
- 不给存储适配器新增通用 `copy` 方法 —— 仅迁移脚本需要，脚本内直接用 OSS client 即可，避免适配器层接口膨胀。
- 不为合同审查新增独立 `FileSource` 枚举值。

## 四、测试

- 为 `buildStorageKey` / `buildStorageDir` 编写单元测试：覆盖 user/system/temp 三种 scope、`basePath` 为空、幂等、`scope='user'` 缺 `userId` 抛错等分支。
