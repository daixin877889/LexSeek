...
- 不改变案件分析（case）/ 文书生成（draft）scope 的**对外行为**。注：为消除重复，案件域的 `caseProcessMaterialMiddleware` 会做等价重构（抽共享进度辅助），属"实现重构、行为不变"，由回归测试（`caseAnalysis.test.ts`）保障。
...
附件清单的 `__ATTACHMENTS__` sentinel 解析：当前 sentinel 有三处实现——`chatQueueActions.ts` 的常量（"写"端）、`useMessageParser.ts` 的常量 + `parseHumanContent` 内联解析（前端"读"端），服务端中间件还要再"读"一次。本次把 **sentinel 常量 + 解析核心 `splitAttachmentSentinel` 下沉到 `shared/utils/`**：写端引用常量，前端 `parseHumanContent` 与服务端中间件都改用 shared 的 `splitAttachmentSentinel`/`parseAttachmentFileIds`——解析逻辑收敛为一份，真正消除漂移（非仅统一常量）。
...
| 加密文件（附件 `encrypted=true`） | 不特殊处理——识别/解密沿用现有 `runRecognitionAndEmbeddingPipeline`（与案件/草稿场景同一条流水线，加密文件的解密能力已内建于该流水线）；本功能不改动其行为 |
...
- **`materialPipeline.service.ts` 体积**：该文件已约 970 行、超 `main.md` 500 行红线（既有技术债）。本设计的会话 pipeline 函数与 draft 版对偶、就近放置（约 +30~40 行）。完整拆分该文件应另立专门重构任务，不在本功能范围内强行处理——避免 scope creep 与回归风险。
