...
| 9 | `memory_extract`（新增配置项） | 案件记忆整理 | 按 token | **停用** | 直接扣（best-effort） | 案件记忆写入钩子 | 新增 |
...
`reserve_points` / `confirm_points` / `rollback_points` 三个工具已注册但未挂载到任何节点，预扣改由统一计费服务驱动后彻底无用。删除三个工具文件，并从 `agent-platform/tools/index.ts` 注销。
...
```
billCheck(userId, itemKey, usage)                  → { sufficient, required, available }
billDirect(userId, itemKey, usage, context)        → { skipped, consumedAmount, operationId }
billReserve(userId, itemKey, usage, context)       → { skipped, batchId, operationId, preDeductAmount }
billSettle(batchId, actualUsage?, context?)        → { consumedAmount }
...
### 5.1 token 计费中间件（场景 1–4）

`pointConsumptionMiddleware` 内部 `afterModel` 改为调用 `billDirect`。

- `beforeAgent` 生成一个 `operationId`（一次 agent 运行一个），存入中间件 state（新增 `_billingOperationId`），`afterModel` 每轮复用。
- `contextLabel` 在 agent 运行装配阶段解析（案件名 / 会话标题），随中间件参数传入。
...
- **`pointBilling.service` 单测**：token / count 两模式、停用跳过、降级容错、`operationId` 复用、各错误分支。
- **场景集成测试**：OCR 扣减、MinerU 预扣→结算/回滚、ASR 走统一服务、摘要/记忆 best-effort 不抛错。
- **聚合查询测试**：多碎记录按 `operationId` 合并、混合状态、空 `operationId` 旧记录兼容。
- `agent-platform/**` 覆盖率维持 ≥90%。
