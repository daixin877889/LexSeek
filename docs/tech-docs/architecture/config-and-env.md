...
| `seo.siteUrl` | `NUXT_PUBLIC_SEO_SITE_URL` | `'https://lexseek.cn'` | 站点 URL，用于 sitemap / canonical 等 |
| `seo.baiduVerify` | `NUXT_PUBLIC_SEO_BAIDU_VERIFY` | `''` | 百度站长验证码 |
| `seo.googleVerify` | `NUXT_PUBLIC_SEO_GOOGLE_VERIFY` | `''` | Google 站长验证码 |
| `seo.bingVerify` | `NUXT_PUBLIC_SEO_BING_VERIFY` | `''` | Bing 站长验证码 |
| `seo.sogouVerify` | `NUXT_PUBLIC_SEO_SOGOU_VERIFY` | `''` | 搜狗站长验证码 |
| `seo.so360Verify` | `NUXT_PUBLIC_SEO_SO360_VERIFY` | `''` | 360 站长验证码 |
...
| `embedding.model` | `NUXT_EMBEDDING_MODEL` | `'text-embedding-v4'` | 嵌入模型名称 |
...
| 配置键 | 环境变量 | 默认值 | 说明 |
|--------|---------|--------|------|
| `mineru.uid` | `NUXT_MINERU_UID` | `''` | MinerU 回调验签 uid（`checksum = SHA256(uid + seed + content)`）；未配置时回调验签降级为 warn-only |
