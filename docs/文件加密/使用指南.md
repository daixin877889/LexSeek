...
// 2. 生成随机恢复密钥（32 字节，Base64 编码）
const randomBytes = new Uint8Array(32)
crypto.getRandomValues(randomBytes)
const recoveryKey = btoa(String.fromCharCode(...randomBytes))
...
// 5. 更新服务器配置
await encryptionStore.resetWithRecoveryKey(
...
**注意**：`clearConfig()` 会调用 `lockIdentity()` 清除 IndexedDB 中的私钥，确保用户切换时不会混淆。
