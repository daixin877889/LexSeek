...
4. 配置 `.env`：`LEGACY_DATABASE_URL`、`DATABASE_URL`；如需给关键账号补绑更细的角色组合，按真实管理员名单填好 `src/adminRoles.ts` 的 `ADMIN_BINDINGS`。
5. `migrate` → `verify`。
6. **案件分析向量重建**：迁移后 `case_analysis_embeddings` 表为空。运行 `npx tsx legacy-migration/reembed.ts` 为历史案件分析（约 9900 条）补齐语义检索向量、补全缺失的分析摘要（幂等、可中断重跑、失败自动重试一轮）。
   案件材料**无需脚本处理**——其识别 / 嵌入 / 摘要由新项目 `caseProcessMaterialMiddleware` 在每次案件分析或小索对话启动前自动补齐（只补缺失项）。
   本步骤可与冒烟测试并行，不阻塞切换。
