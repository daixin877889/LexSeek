-- ==================== 重建 M3/M4 HNSW 向量索引 ====================
-- 背景：上一个迁移 20260426064020_add_skills_and_node_skills_use_skills_as_logic
-- 因 Prisma schema 与数据库存在 schema drift（HNSW 索引由独立 SQL 创建，
-- 未在 prisma/models/*.prisma 中声明），prisma migrate dev 自动生成了
-- DROP INDEX 语句把两个 HNSW 索引清掉，导致 case_memories / case_analysis_embeddings
-- 的向量检索性能退化。本迁移立即重建。
--
-- 注意：HNSW 索引无法用 Prisma schema 表达（pgvector 扩展类型），所以本项目
-- 通过 raw SQL migration 维护它们；每次 prisma migrate dev 都会再次检测为 drift
-- 并生成 DROP——这是已知的项目级权衡。后续 migrate 工作流需要纪律性地
-- 配套 restore migration，或在 prisma migrate 命令外维护索引。
--
-- 安全性：
--   * IF NOT EXISTS 幂等
--   * 当前数据量小，秒级完成
