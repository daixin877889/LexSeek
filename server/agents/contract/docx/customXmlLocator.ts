/**
 * 在 docx zip 内按命名空间 URI + 根元素名定位 LexSeek 写入的 customXml 身份证文件。
 *
 * 为什么不按固定路径找：LexSeek 导出时把身份证写在 word/customXml/xxx.xml，
 * 但 Microsoft Word 重新保存 docx 会按 OOXML 规范把 customXml part 移到包根
 * customXml/ 并改名为 item{N}.xml。固定路径查找失效。
 *
 * 为什么命中 URI 后还要 parse 确认根元素：Word 会为每个 customXml part 生成配套的
 * itemProps{N}.xml（properties），其中 <ds:schemaRef ds:uri="..."> 会带上该 part
 * 的命名空间 URI。仅靠 URI 字符串匹配会把 itemProps 误判为身份证文件，故命中 URI
 * 后还需 parse 确认根元素本地名。
 */
import type { DocxZip } from './zipRewriter'
import { parseOoxml, tagOf } from './xmlAst'

/** 批注身份证（annotationRefs）根命名空间与根元素名 */
export const ANNOTATION_REFS_NS = 'urn:lexseek:contract-review:v1'
export const ANNOTATION_REFS_ROOT = 'lexseekAnnotationRefs'
/** 修订身份证（redlineRefs）根命名空间与根元素名 */
export const REDLINE_REFS_NS = 'urn:lexseek:contract-review-redline:v1'
export const REDLINE_REFS_ROOT = 'lexseekRedlineRefs'
...
/**
 * 遍历 docx zip 内所有 customXml part，返回「命名空间 URI 命中 且 根元素本地名匹配」
 * 的第一个。
 *
 * @param zip           已 loadDocxZip 的 docx
 * @param namespaceUri  LexSeek 专有命名空间 URI（见本文件常量）
 * @param rootLocalName 身份证根元素本地名（剥掉命名空间前缀后比对，见本文件常量）
...
export async function locateLexseekCustomXml(
    ...
    for (const file of candidates) {
        ...
        if (!xml.includes(namespaceUri)) continue
        // URI 命中后再 parse 确认根元素 —— 排除 itemProps*.xml（其 schemaRef 也含该
        // URI，但根元素是 ds:datastoreItem）。比对剥掉可能的命名空间前缀后的本地名。
        try {
            const ast = parseOoxml(xml)
            for (const node of ast) {
                const tag = tagOf(node)
                if (tag && tag.replace(/^[^:]*:/, '') === rootLocalName) {
                    return { xml, path: file.name, atOriginalPath: file.name === originalPath }
                }
            }
        } catch { /* XML 解析失败，跳过该文件 */ }
    }
    return null
}
