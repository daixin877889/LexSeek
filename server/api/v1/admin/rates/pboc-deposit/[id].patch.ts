/**
 * PATCH /api/v1/admin/rates/pboc-deposit/:id
 *
 * 管理端：更新央行存款基准利率记录。
 * 权限由 server/middleware/03.permission.ts 统一 RBAC 拦截。
 */
import { z } from 'zod'
import { updatePBOCDepositRateService } from '~~/server/services/rates/rates.service'

const schema = z.object({
    effectDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '日期格式必须为 YYYY-MM-DD').optional(),
    demand: z.number().min(0).max(99.9999).optional(),
    threeMonths: z.number().min(0).max(99.9999).optional(),
    sixMonths: z.number().min(0).max(99.9999).optional(),
    oneYear: z.number().min(0).max(99.9999).optional(),
    twoYear: z.number().min(0).max(99.9999).optional(),
    threeYear: z.number().min(0).max(99.9999).optional(),
...
export default defineEventHandler(async (event) => {
    ...
    try {
        const data = await updatePBOCDepositRateService(id, result.data)
        return resSuccess(event, '更新成功', data)
    } catch (err: any) {
        logger.error('[admin] 更新 PBOC 存款利率失败', err)
