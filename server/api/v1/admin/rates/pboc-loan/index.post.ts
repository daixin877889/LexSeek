/**
 * POST /api/v1/admin/rates/pboc-loan
 *
 * 管理端：新增央行贷款基准利率记录。
 * 权限由 server/middleware/03.permission.ts 统一 RBAC 拦截。
 */
import { z } from 'zod'
import { createPBOCLoanRateService } from '~~/server/services/rates/rates.service'
...
export default defineEventHandler(async (event) => {
    ...
    try {
        const data = await createPBOCLoanRateService(result.data)
        return resSuccess(event, '创建成功', data)
    } catch (err: any) {
        logger.error('[admin] 创建 PBOC 贷款利率失败', err)
