import { getCaseAnalysisWorkflow } from '~~/server/services/workflow/caseAnalysisV2.workflow'
...
export default defineEventHandler(async (event) => {
  ...
  // 获取已配置 checkpointer 的工作流
  const workflow = await getCaseAnalysisWorkflow()

  // 使用 encoding: 'text/event-stream' 直接产出 SSE 流
  ...
  // LangGraph encoding: 'text/event-stream' 已经产出标准 SSE 流，直接返回
  return stream
})
