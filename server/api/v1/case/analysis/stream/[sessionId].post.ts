...
/** 将 BaseMessage 实例转为前端 SDK 期望的 plain dict 格式，过滤 RemoveMessage */
function serializeMessages(messages: unknown[]): unknown[] {
    return messages
        .filter((msg): msg is { getType: () => string } => isBaseMessage(msg))
        .filter(msg => msg.getType() !== 'remove')
        .map(msg => {
            const { type, data } = (msg as any).toDict()
            return { ...data, type }
        })
}

/** 序列化 values payload：将 messages 数组转为 plain dict 格式 */
function serializeValuesPayload(payload: unknown): unknown {
    if (!payload || typeof payload !== 'object') return payload
    const obj = payload as Record<string, unknown>
    if (!Array.isArray(obj.messages)) return payload
    return { ...obj, messages: serializeMessages(obj.messages) }
}

/** 序列化 messages payload：[MessageChunk, metadata] 元组 */
function serializeMessagesPayload(payload: unknown): unknown {
    if (!Array.isArray(payload) || payload.length < 1) return payload
    const [msg, metadata] = payload
    if (!isBaseMessage(msg)) return payload
    const { type, data } = (msg as any).toDict()
...
export default defineEventHandler(async (event) => {
    ...
    const thinking = body?.thinking !== false  // 默认启用
    ...
    // 调用 mainAgent 获取 LangGraph stream 流
    ...
    // 返回 LangGraph 标准 SSE 流
    // FetchStreamTransport + StreamManager 期望 event: <type>\ndata: <json>\n\n 格式
    // agent.stream() 多 mode 下每个 chunk 为 [mode, payload]
        ...
        async start(controller) {
            ...
            try {
                for await (const chunk of agentStream) {
                    ...
                    // values 事件包含完整状态，messages 事件包含消息 chunk
                    // 两者都需要将 BaseMessage 实例转为 plain dict
                    const serialized = mode === 'values'
                        ? serializeValuesPayload(payload)
                        : mode === 'messages'
                            ? serializeMessagesPayload(payload)
                            : payload
                ...
                const errorData = JSON.stringify({ error: '分析过程中发生错误' })
