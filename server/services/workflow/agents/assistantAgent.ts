...
export async function runAssistantChat(
    ...
    // 4. 构建系统提示词（assistant 场景 caseId=null：buildContextSegments 退化为单段
    //    roleAndFlow，仍走 cache 架构以与其他 agent 保持一致）
    const roleAndFlow = renderSystemPrompt(mainConfig, {})
    const { systemMessage: systemPrompt, plainText: systemPromptText } = await buildSystemPromptForAgent(
