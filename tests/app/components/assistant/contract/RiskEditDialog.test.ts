 ...
 *
 * - 新增 / 编辑单条 Risk
 * - 按 RISK_SHAPE 做前端一层校验（clauseIndex ≥ 0 整数；必填非空；high/medium 必含 suggestedClauseText）
...
function mountDialog(props: Partial<{ open: boolean; risk: Risk | null }> = {}) {
...
const numberInput = (w: Wrapper) =>
    w.findAll('input').find(i => (i.element as HTMLInputElement).type === 'number')!
...
// textarea 顺序：clauseText, problem, legalBasis, analysis, risk, suggestion, suggestedClauseText
async function fillValidForm(w: Wrapper) {
    const tas = w.findAll('textarea')
    const values = ['原条款文本', '问题概述文本', '法律依据文本', '条款分析文本', '法律风险文本', '修改建议文本', '建议改写后的完整条款']
    for (let i = 0; i < values.length; i++) await tas[i]!.setValue(values[i])
    await textInput(w).setValue('付款条件')
    await numberInput(w).setValue('2')
}

describe('RiskEditDialog', () => {
    it('新增模式（risk=null）：默认 level=medium，其他字段为空', async () => {
    ...
    it('编辑模式（risk 非空）：9 个字段被预填', async () => {
        const r = baseRisk()
        const w = mountDialog({ risk: r })
        await nextTick()
        expect(w.text()).toContain('编辑风险')
        const tas = w.findAll('textarea')
        const expected = [r.clauseText, r.problem, r.legalBasis, r.analysis, r.risk, r.suggestion, r.suggestedClauseText]
        expected.forEach((v, i) => expect((tas[i]!.element as HTMLTextAreaElement).value).toBe(v))
        expect((numberInput(w).element as HTMLInputElement).value).toBe(String(r.clauseIndex))
    ...
    it('high 级别且 suggestedClauseText 为空：确认 disabled', async () => {
        ...
        await w.findAll('textarea')[6]!.setValue('')
        await nextTick()
        expect((confirmBtn(w).element as HTMLButtonElement).disabled).toBe(true)
    })
    ...
    it.each([
        { name: 'clauseIndex=-1', fn: async (w: Wrapper) => { await fillValidForm(w); await pickLevel(w, 'low'); await numberInput(w).setValue('-1') } },
        { name: 'clauseIndex=1.5', fn: async (w: Wrapper) => { await fillValidForm(w); await pickLevel(w, 'low'); await numberInput(w).setValue('1.5') } },
        { name: '必填留空', fn: async (w: Wrapper) => { await pickLevel(w, 'low') } },
    ])('$name：确认 disabled', async ({ fn }) => {
        const w = mountDialog({ risk: null })
        await fn(w)
        await nextTick()
        expect((confirmBtn(w).element as HTMLButtonElement).disabled).toBe(true)
    ...
    it('确认（新增）：emit confirm 携带 uuid + update:open(false)', async () => {
        const w = mountDialog({ risk: null })
        await fillValidForm(w)
        ...
        expect(payload.clauseIndex).toBe(2)
    ...
    it('legalBasis / suggestedClauseText 空白字符串：payload 为 undefined', async () => {
        ...
        await fillValidForm(w)
        await pickLevel(w, 'low')
        await w.findAll('textarea')[2]!.setValue('   ')
        await w.findAll('textarea')[6]!.setValue('')
        await nextTick()
        await confirmBtn(w).trigger('click')
