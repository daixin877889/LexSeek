...
describe('节点 DAO 测试', () => {
    ...
    afterAll(async () => {
        ...
        try {
            const testNodeIds = (await testPrisma.nodes.findMany({ where: { OR: [{ name: { startsWith: 'test_node_' } }, { name: { startsWith: 'node_test_' } }] }, select: { id: true } })).map(n => n.id)
            ...
            await testPrisma.nodes.deleteMany({ where: { OR: [{ name: { startsWith: 'test_node_' } }, { name: { startsWith: 'node_test_' } }] } })
